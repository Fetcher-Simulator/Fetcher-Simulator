    ...
    for (const std::string& cellId : cells)
    {
        ...
        for (const auto& [actorKey, record] : cellIt->second.actors)
        {
            ...
            if (const ESM::NPC* npc = mContentRegistry->store().get<ESM::NPC>().search(refId))
            {
                ...
                if (request.offenderCharacterId > 0)
                {
                    const auto hostilityIt = record.crimeHostilities.find(request.offenderCharacterId);
                    if (hostilityIt != record.crimeHostilities.end())
                    {
                        actor.fight = hostilityIt->second.fight;
                        actor.fightProvenance = CrimeFightProvenance::ServerOffenderScoped;
                    }
                }
...
void MPServer::dispatchCrimeReactions(ConnectedClient& offender, const CrimeSemanticResult& result)
{
    ...
    for (const CrimeWitnessResult& witness : result.witnesses)
    {
        ...
        else if (witness.aggression.combat)
        {
            ...
            CrimeHostilityState& hostility = record.crimeHostilities[offender.dbCharacterId];
            if (hostility.crimeId < 0)
                hostility.baseFight = witness.aggression.baseFight;
            hostility.lastGuid = offender.guid;
            hostility.fight = witness.aggression.finalFight;
            hostility.crimeId = result.state.currentCrimeId;
            hostility.liveCombat = true;
    ...
    const std::string& cellId, std::int64_t onlyCharacterId)
{
    ...
    std::unordered_map<std::int64_t, CrimeReactionDirective> hostilityDirectives;
    for (auto& [actorKey, record] : cellIt->second.actors)
    {
        (void)actorKey;
        if (record.actor.isDead || record.migrationGeneration == 0 || record.crimeHostilities.empty())
            continue;

        std::vector<std::int64_t> characterIds;
        characterIds.reserve(record.crimeHostilities.size());
        for (const auto& [characterId, hostility] : record.crimeHostilities)
        {
            (void)hostility;
            if (onlyCharacterId <= 0 || characterId == onlyCharacterId)
                characterIds.push_back(characterId);
        }
        std::sort(characterIds.begin(), characterIds.end());

        for (const std::int64_t characterId : characterIds)
        {
            ConnectedClient* offender = findOffender(characterId);
            if (!offender || offender->player.isDead
                || makeCellKey(offender->player.cell) != cellId)
                continue;

            CrimeHostilityState& hostility = record.crimeHostilities.at(characterId);
            ensureActorNetId(record, cellId);
            if (record.actorNetId == 0)
                break;

            const std::string targetId = crimeReactionOffenderTargetId(offender->guid);
            record.actor.ai.type = BaseActor::AIAction::Type::Combat;
            record.actor.ai.targetId = targetId;
            record.actor.ai.targetMpNum = 0;
            record.actor.ai.duration = 0.f;
            ...
            CrimeReactionDirective& directive = hostilityDirectives[characterId];
            if (directive.eventId.empty())
            {
                directive.eventId = "crime-hostility:" + std::to_string(characterId)
                    + ':' + std::to_string(nowMs);
                directive.cellId = cellId;
                directive.offenderGuid = offender->guid;
            }
            ...
            // One actor can execute only one combat package at a time. Keep any
            // additional offender-scoped hostility records dormant until their
            // offender is explicitly re-evaluated/re-enters later.
            break;
        }
    }
    ...
    for (auto& [characterId, directive] : hostilityDirectives)
    {
        (void)characterId;
        if (!validateCrimeReactionDirective(directive))
        {
            Log(Debug::Error) << "[CrimeReaction] refusing invalid ordinary-hostility directive"
                              << " event=" << directive.eventId
                              << " cell=" << cellId
                              << " actors=" << directive.actors.size();
            continue;
        ...
        packet.directive = directive;
        broadcastActorToCell(cellId, packet.encode());
        Log(Debug::Info) << "[CrimeReaction] reissued ordinary crime hostility"
                         << " event=" << directive.eventId
                         << " cell=" << cellId
                         << " actors=" << directive.actors.size()
                         << " offenderGuid=" << directive.offenderGuid;
    ...
    ConnectedClient& offender, const std::string& cellId)
{
    ...
    {
        (void)actorKey;
        const bool guardMatch = record.crimePursuitCharacterId == offender.dbCharacterId;
        auto hostilityIt = record.crimeHostilities.find(offender.dbCharacterId);
        const bool hostilityMatch = hostilityIt != record.crimeHostilities.end();
        if (!guardMatch && !hostilityMatch)
            continue;

        std::uint32_t targetGuid = offender.guid;
        if (guardMatch)
        {
            if (record.crimeEnforcementState == CrimeEnforcementState::ArrestPending)
            {
            ...
            record.crimePursuitReassertArmed = false;
            if (record.crimePursuitLastGuid != 0)
                targetGuid = record.crimePursuitLastGuid;
        }
        if (hostilityMatch)
        {
            hostilityIt->second.liveCombat = false;
            if (hostilityIt->second.lastGuid != 0)
                targetGuid = hostilityIt->second.lastGuid;
        }

        const std::string targetId = crimeReactionOffenderTargetId(targetGuid);
    ...
    if (!validateCrimeReactionDirective(directive))
    {
        Log(Debug::Error) << "[CrimeReaction] refusing invalid suspended-enforcement directive"
                          << " event=" << directive.eventId
                          << " cell=" << cellId
                          << " actors=" << directive.actors.size();
        return;
    ...
    packet.directive = directive;
    broadcastActorToCell(cellId, packet.encode());
    Log(Debug::Info) << "[CrimeReaction] suspended live crime enforcement"
                     << " event=" << directive.eventId
                     << " cell=" << cellId
                     << " actors=" << directive.actors.size()
                     << " offenderGuid=" << offender.guid
...
void MPServer::clearOutstandingCrimePursuitsForCharacter(ConnectedClient& offender)
{
    ...
    const std::uint64_t nowMs = currentServerTimeMs();
    std::unordered_map<std::string, CrimeReactionDirective> directives;
    std::size_t clearedGuards = 0;
    std::size_t clearedHostilities = 0;
    ...
    for (auto& [cellId, cellState] : mWorld.actorCells)
    {
        for (auto& [actorKey, record] : cellState.actors)
        {
            ...
            std::uint32_t lastGuid = offender.guid;
            if (guardMatch && record.crimePursuitLastGuid != 0)
                lastGuid = record.crimePursuitLastGuid;
            if (hostilityMatch && hostilityIt->second.lastGuid != 0)
                lastGuid = hostilityIt->second.lastGuid;
            ...
            if (hostilityMatch)
            {
                record.crimeHostilities.erase(hostilityIt);
                ++clearedHostilities;
            }

            const std::string lastTargetId = crimeReactionOffenderTargetId(lastGuid);
    ...
    for (auto& [cellId, directive] : directives)
    {
        if (!validateCrimeReactionDirective(directive))
        {
            Log(Debug::Error) << "[CrimeReaction] refusing invalid clear-enforcement directive"
                              << " event=" << directive.eventId
                              << " cell=" << cellId
                              << " actors=" << directive.actors.size();
            continue;
        ...
        packet.directive = directive;
        broadcastActorToCell(cellId, packet.encode());
        Log(Debug::Info) << "[CrimeReaction] cleared outstanding crime enforcement"
                         << " event=" << directive.eventId
                         << " cell=" << cellId
                         << " actors=" << directive.actors.size()
                         << " offenderGuid=" << offender.guid;
    ...
    if (clearedGuards != 0 || clearedHostilities != 0)
    {
        Log(Debug::Info) << "[CrimeReaction] cleared offender crime lifecycle"
                         << " characterId=" << offender.dbCharacterId
                         << " offenderGuid=" << offender.guid
                         << " guards=" << clearedGuards
                         << " ordinaryHostilities=" << clearedHostilities
                         << " bounty=" << offender.player.crimeState.bounty;
    }
}
...
std::vector<MPServer::LiveObservationDiagnostic> MPServer::observeNpcCandidates(
    ...
    std::string_view eventId, float maximumDistance, bool guardsOnly)
{
    ...
    const float resolvedMaximumDistance = maximumDistance < 0.f
        ? mObservationAlarmRadius : maximumDistance;
    const bool limitDistance = resolvedMaximumDistance > 0.f;
    const float maximumDistanceSquared = resolvedMaximumDistance * resolvedMaximumDistance;
    ...
    for (const std::string& cellId : collisionCells)
    {
        ...
        for (const auto& [actorKey, record] : cellIt->second.actors)
        {
            ...
            const ESM::RefId refId = ESM::RefId::stringRefId(record.actor.refId);
            const ESM::NPC* npc = mContentRegistry->store().get<ESM::NPC>().search(refId);
            if (npc == nullptr || (guardsOnly && npc->mClass != "guard"))
                continue;
            ...
            if (!std::isfinite(distanceSquared)
                || (limitDistance && distanceSquared > maximumDistanceSquared))
                continue;
...
void MPServer::evaluateGuardCrimeEnforcement(ConnectedClient& offender, std::uint64_t nowMs)
{
    constexpr std::uint64_t ScanIntervalMs = 500;
    if (!mContentRegistry || !mObservationService || offender.dbCharacterId <= 0
        || !offender.charSelectComplete || offender.player.isDead)
        return;
    if (nowMs < offender.crimeEnforcementRespawnGraceUntilMs)
        return;
    if (offender.lastGuardCrimeScanMs != 0
        && nowMs - offender.lastGuardCrimeScanMs < ScanIntervalMs)
        return;
    offender.lastGuardCrimeScanMs = nowMs;

    const auto& settings = mContentRegistry->store().get<ESM::GameSetting>();
    const std::int32_t crimeThreshold = settings.find("iCrimeThreshold")->mValue.getInteger();
    const std::int32_t thresholdMultiplier
        = settings.find("iCrimeThresholdMultiplier")->mValue.getInteger();
    const GuardCrimeEnforcementDecision decision = calculateGuardCrimeEnforcement(
        offender.player.crimeState.bounty, crimeThreshold, thresholdMultiplier);
    if (decision == GuardCrimeEnforcementDecision::None)
        return;

    const std::string offenderCell = makeCellKey(offender.player.cell);
    if (offenderCell.empty())
        return;

    const std::string eventId = "guard-enforcement:" + std::to_string(offender.dbCharacterId)
        + ':' + std::to_string(nowMs);
    const std::vector<LiveObservationDiagnostic> diagnostics
        = observeNpcCandidates(offender.guid, std::nullopt, eventId, 0.f, true);
    if (diagnostics.empty())
        return;

    std::unordered_map<std::string, CrimeReactionDirective> directives;
    for (const LiveObservationDiagnostic& diagnostic : diagnostics)
    {
        if (!diagnostic.result.observable)
            continue;

        const auto keyIt = mWorld.actorKeysByNetId.find(diagnostic.observerActorNetId);
        if (keyIt == mWorld.actorKeysByNetId.end())
            continue;
        const auto locationIt = mWorld.actorLocations.find(keyIt->second);
        if (locationIt == mWorld.actorLocations.end())
        ...
        if (cellIt == mWorld.actorCells.end())
            continue;
        auto recordIt = cellIt->second.actors.find(keyIt->second);
        ...
        const ESM::NPC* npc = mContentRegistry->store().get<ESM::NPC>().search(
            ESM::RefId::stringRefId(record.actor.refId));
        if (!npc || npc->mClass != "guard")
            continue;

        // Native updateCrimePursuit only originates a new enforcement package
        // for an idle guard. Existing combat/pursuit, including another player's
        // server-authored encounter, wins until it is resolved.
        if (record.actor.ai.type == BaseActor::AIAction::Type::Combat
            || record.actor.ai.type == BaseActor::AIAction::Type::Pursue)
            continue;
        if (record.crimePursuitCharacterId > 0
            && record.crimePursuitCharacterId != offender.dbCharacterId)
        ...
        ensureActorNetId(record, locationIt->second);
        if (record.actorNetId == 0)
            continue;

        record.crimePursuitCharacterId = offender.dbCharacterId;
        record.crimePursuitLastGuid = offender.guid;
        record.crimeEnforcementState = decision == GuardCrimeEnforcementDecision::Combat
            ? CrimeEnforcementState::Combat : CrimeEnforcementState::Arrest;
        record.crimePursuitReassertArmed = false;
        record.crimePursuitLastReassertMs = nowMs;
        record.actor.ai.type = decision == GuardCrimeEnforcementDecision::Combat
            ? BaseActor::AIAction::Type::Combat : BaseActor::AIAction::Type::Pursue;
        record.actor.ai.targetId = crimeReactionOffenderTargetId(offender.guid);
        record.actor.ai.targetMpNum = 0;
        record.actor.ai.duration = 0.f;
        record.actor.ai.reset = false;
        record.lastSnapshotTime = nowMs;
        ...
        if (directive.eventId.empty())
        {
            directive.eventId = eventId;
            directive.cellId = locationIt->second;
            directive.offenderGuid = offender.guid;
        }
        const std::uint8_t flags = decision == GuardCrimeEnforcementDecision::Combat
            ? static_cast<std::uint8_t>(CrimeReactionSetAlarmed | CrimeReactionStartCombat)
            : static_cast<std::uint8_t>(CrimeReactionSetAlarmed | CrimeReactionPursueOffender);
        directive.actors.push_back({ record.actorNetId, record.migrationGeneration,
            CrimeReactionDialogue::None, flags });
    }
    ...
    for (auto& [cellId, directive] : directives)
    {
        if (!validateCrimeReactionDirective(directive))
        {
            Log(Debug::Error) << "[CrimeReaction] refusing invalid guard encounter directive"
                              << " event=" << directive.eventId
                              << " cell=" << cellId
                              << " actors=" << directive.actors.size();
            continue;
        ...
        packet.directive = directive;
        broadcastActorToCell(cellId, packet.encode());
        Log(Debug::Info) << "[CrimeReaction] server authorized fresh guard encounter"
                         << " event=" << directive.eventId
                         << " cell=" << cellId
                         << " actors=" << directive.actors.size()
                         << " offenderGuid=" << offender.guid
                         << " characterId=" << offender.dbCharacterId
                         << " bounty=" << offender.player.crimeState.bounty
                         << " threshold=" << crimeThreshold
                         << " combatThreshold="
                         << static_cast<std::int64_t>(crimeThreshold) * thresholdMultiplier
                         << " decision="
                         << (decision == GuardCrimeEnforcementDecision::Combat ? "combat" : "arrest");
    }
}
...
bool MPServer::handleCrimeWitnessDiagnosticCommand(ConnectedClient& requester, std::string_view message)
{
    ...
    request.offenderCharacterId = requester.dbCharacterId;
        ...
        else if (snapshot.kind == MechanicsSubjectKind::Player && mPlayerDb)
        {
            try
            {
                ...
                if (accepted)
                {
                    ...
                    evaluateGuardCrimeEnforcement(c, receivedAtMs);
                }
            }
            catch (const std::exception& e)
            {
    ...
    bool isWerewolf, std::uint64_t observedAtMs)
{
    ...
    if (transformed)
    {
        ...
        request.offenderCharacterId = c.dbCharacterId;
        request.alarmRadius = mObservationAlarmRadius;
                               ...
                               const uint8_t* data, size_t size)
{
    ...
    switch (type)
    {
        ...
        case PacketType::InventoryTakeBatchRequest: handleInventoryTakeBatchRequest(client, data, size); break;
...
void MPServer::handlePlayerCellChange(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    Log(Debug::Info) << "[Server] " << c.name << " Ã¢â€ â€™ cell: " << newCell;
...
void MPServer::handlePlayerDeath(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    c.crimeEnforcementRespawnGraceUntilMs = 0;
    c.lastGuardCrimeScanMs = 0;
...
void MPServer::handlePlayerResurrect(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    // Multiplayer respawn has no vanilla save/load equivalent. Give the player
    // a short control window before a fresh server-authorized guard encounter
    // can be established, otherwise a high bounty plus shrine-adjacent guards
    // creates an unavoidable death/resurrect loop.
    constexpr std::uint64_t CrimeRespawnGraceMs = 10000;
    const std::uint64_t nowMs = currentServerTimeMs();
    c.crimeEnforcementRespawnGraceUntilMs = nowMs + CrimeRespawnGraceMs;
    c.lastGuardCrimeScanMs = 0;
    broadcastToAll(std::vector<uint8_t>(data, data + size), c.conn);

    Log(Debug::Info) << "[Server] Relayed PlayerResurrect for " << c.name
                     << " crimeEnforcementGraceMs=" << CrimeRespawnGraceMs;
    syncLuaPlayerSnapshot();
}
...
void MPServer::handleActorList(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    for (auto& actor : incoming.actors)
    {
        ...
        if (previousRecord)
        {
            ...
            it->second.crimeHostilities = previousRecord->crimeHostilities;
        }
    ...
    for (const ActorRegistryRecord& previousRecord : previousCellRecords)
    {
        if (previousRecord.actor.mpNum != 0 || previousRecord.actor.isDead
            || (previousRecord.crimePursuitCharacterId <= 0
                && previousRecord.crimeHostilities.empty()))
            continue;
...
void MPServer::handleActorEquipment(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    for (auto& actor : incoming.actors)
    {
        ...
        stored->actor.mpNum = actor.mpNum;
        stored->actor.cellId = incoming.cellId;
        const std::vector<EquipmentItem> previousEquipment = stored->actor.equipment;
        stored->actor.equipment = actor.equipment;

        const std::string containerKey = makeContainerKey(
            incoming.cellId, stored->actor.refId, stored->actor.refNum, stored->actor.mpNum);
        auto containerIt = mWorld.containers.find(containerKey);
        if (containerIt != mWorld.containers.end() && containerIt->second.hasAuthority
            && reconcileAuthoritativeContainerEquipment(
                containerIt->second.items, previousEquipment, actor.equipment))
        {
            if (mPlayerDb)
                mPlayerDb->upsertContainerRecord(containerIt->second);
            Log(Debug::Verbose) << "[Server] Reconciled actor equipment metadata into authoritative container"
                                << " refId=" << stored->actor.refId
                                << " refNum=" << stored->actor.refNum
                                << " mpNum=" << stored->actor.mpNum
                                << " cell=" << incoming.cellId;
...
void MPServer::handleActorCombatResult(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    if (qualifyingCrime || murderAttributable)
    {
        ...
        witnessRequest.offenderCharacterId = attacker->dbCharacterId;
...
void MPServer::handleWorldItemTakeRequest(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    if (result.theft && mObservationService)
    {
        ...
        witnessRequest.offenderCharacterId = c.dbCharacterId;
        witnessRequest.alarmRadius = mObservationAlarmRadius;
        witnessRequest.observedAtMs = nowMs;
        witnessRequest.maximumSnapshotAgeMs = MaximumSnapshotAgeMs;
        CrimeWitnessBuildResult witnesses = buildLiveCrimeWitnesses(witnessRequest);
...
void MPServer::handleInventoryTakeRequest(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    if (!finish)
    {
        // The helper selects the exact source row independently of this predicate;
        // native stacking only permits aggregation/equivalent stack representation.
    ...
    if (!finish && !aggregateTake)
    {
        ...
        for (const ContainerItem& item : resultingSource.items)
        {
            if (!first)
                available << ',';
            first = false;
            available << item.refId << "[instanceId=" << item.instanceId
                      << ",count=" << item.count << ",charge=" << item.charge
        ...
        Log(Debug::Warning) << "[InventoryTake] item unavailable request=" << request.requestId
                            ...
                            << " enchant=" << request.itemEnchantmentCharge
                            << " soul=" << request.itemSoul
    ...
    if (!detected && !finish)
    {
        ...
        // Exact removal of a non-stackable row is valid, but the acquired item
        // must still receive its own player-inventory identity. Use two native
        // representatives of the actual source metadata to test stackability.
        const bool allowTakeStack = authoritativeContainerItemsStackNatively(
            mContentRegistry->store(), removedSourceItem, removedSourceItem);
        ...
        const bool compatibleStack = allowTakeStack && std::any_of(inventory.begin(), inventory.end(),
            [&](const Item& item) {
        ...
        const AuthoritativeStackMutation takeMutation
            = mergeAuthoritativeInventoryItem(inventory, added, allowTakeStack, canStackIntoPlayerItem);
...
// ---------------------------------------------------------------------------
void MPServer::handleInventoryTakeBatchRequest(ConnectedClient& c, const uint8_t* data, size_t size)
{
    PacketInventoryTakeBatchRequest packet;
    if (!packet.decode(data, size))
        return;

    const InventoryTakeBatchRequest& request = packet.request;
    InventoryTakeBatchResult result;
    result.requestId = request.requestId;
    result.kind = request.kind;
    result.source = request.source;
    result.lineCount = static_cast<std::uint32_t>(request.items.size());
    for (const InventoryTakeBatchLine& line : request.items)
        result.itemCount += line.requestedCount;

    auto sendResult = [&] {
        PacketInventoryTakeBatchResult response;
        response.result = result;
        sendTo(c.conn, response.encode());
    };
    auto reject = [&](InventoryTakeError error) {
    ...
    const InventoryTakeError validation = validateInventoryTakeBatchRequest(request);
    if (validation != InventoryTakeError::None || !mPlayerDb || !mContentRegistry
        || c.dbAccountId <= 0 || c.dbCharacterId <= 0)
    {
        reject(validation != InventoryTakeError::None ? validation : InventoryTakeError::PersistenceFailure);
    ...
    const std::string requestHash = crypto::sha256hex(canonicalInventoryTakeBatchRequest(request));
    if (const auto stored = mPlayerDb->loadInventoryTake(c.dbAccountId, c.dbCharacterId, request.requestId))
    {
        ...
        result.theft = stored->result.theft;
        result.crimeValue = stored->result.crimeValue;
        const std::string sourceKey = makeContainerKey(request.source.cellId, request.source.refId,
            request.source.refNum, request.source.mpNum);
        const auto sourceIt = mWorld.containers.find(sourceKey);
        if (sourceIt != mWorld.containers.end() && sourceIt->second.hasAuthority)
        {
            PacketContainer correction;
            correction.container = sourceIt->second;
            correction.authorityGeneration = currentActorAuthorityGenerationForCell(sourceIt->second.cellId);
    ...
    {
        if (request.kind != InventoryTakeKind::Container || request.source.mpNum != 0)
        {
            reject(InventoryTakeError::StaleSource);
            return;
        }
    ...
    if (!ensureContainerItemIdentities(sourceIt->second))
    {
        reject(InventoryTakeError::PersistenceFailure);
        return;
    }
    ...
    const AcceptedMechanicsSnapshot* acceptedVictim = nullptr;
    if (actorSource && request.kind == InventoryTakeKind::ActorInventory)
    {
        acceptedVictim = mMechanicsSnapshots.findFresh(
            { MechanicsSubjectKind::Npc, 0, request.source.actorInstanceId }, nowMs, MaximumSnapshotAgeMs);
        if (!acceptedVictim
    ...
    const bool theft = request.kind != InventoryTakeKind::Corpse
        && ((actorSource && !actorAllowsUse)
            || (!actorSource && !ownershipGlobalAllowsUse && (!ownerAllowsUse || !factionAllowsUse)));
    ...
    for (const InventoryTakeBatchLine& line : request.items)
    {
        auto aggregateTake = takeAuthoritativeContainerItems(resultingSource.items, line.itemRefId,
            line.itemCharge, line.itemEnchantmentCharge, line.itemSoul, line.requestedCount,
            [&](const ContainerItem& left, const ContainerItem& right) {
                return authoritativeContainerItemsStackNatively(mContentRegistry->store(), left, right);
            }, line.itemInstanceId);
        if (!aggregateTake)
        {
            Log(Debug::Warning) << "[InventoryTakeBatch] item unavailable request=" << request.requestId
                                << " source=" << request.source.refId
                                << " item=" << line.itemRefId
                                << " instanceId=" << line.itemInstanceId
                                << " charge=" << line.itemCharge
                                << " enchant=" << line.itemEnchantmentCharge;
            reject(InventoryTakeError::ItemUnavailable);
            return;
        }

        const ContainerItem removedSourceItem = aggregateTake->taken;
        Log(Debug::Info) << "[InventoryTakeBatch] selected request=" << request.requestId
                         << " source=" << request.source.refId
                         << " requestedRefId=" << line.itemRefId
                         << " requestedInstanceId=" << line.itemInstanceId
                         << " authoritativeRefId=" << removedSourceItem.refId
                         << " authoritativeInstanceId=" << removedSourceItem.instanceId
                         << " count=" << line.requestedCount
                         << " charge=" << removedSourceItem.charge
                         << " enchant=" << removedSourceItem.enchantmentCharge
                         << " soul=" << removedSourceItem.soul;
        removedSourceItems.insert(removedSourceItems.end(),
            aggregateTake->backingRows.begin(), aggregateTake->backingRows.end());
        ...
        try
        {
            ...
            MWWorld::ManualRef contentRef(mContentRegistry->store(), contentId, line.requestedCount);
            MWWorld::Ptr ptr = contentRef.getPtr();
            itemValue = ptr.getClass().getValue(ptr);
            gold = ptr.getClass().isGold(ptr);
            added.refId = gold ? "gold_001" : line.itemRefId;
            added.count = line.requestedCount;
            added.charge = removedSourceItem.charge;
            added.enchantmentCharge = removedSourceItem.enchantmentCharge;
            added.soul = removedSourceItem.soul;
        }
        ...
        const AuthoritativeStackMutation mutation
            = mergeAuthoritativeInventoryItem(inventory, added, allowTakeStack, canStackIntoPlayerItem);
        if (mutation == AuthoritativeStackMutation::Invalid
            || mutation == AuthoritativeStackMutation::Overflow)
        {
            reject(mutation == AuthoritativeStackMutation::Overflow
                ? InventoryTakeError::InvalidCount : InventoryTakeError::PersistenceFailure);
            return;
        }
        ...
        if (theft)
        {
            crimeValue += gold ? static_cast<std::int64_t>(line.requestedCount)
                               : static_cast<std::int64_t>(line.requestedCount) * itemValue;
            if (!gold)
            {
                if (actorSource)
                    stolenItemMutations.push_back({ added.refId, actorRecord->actor.refId, false, line.requestedCount });
                else if (!ownerId.empty())
                    stolenItemMutations.push_back({ added.refId, ownerId, false, line.requestedCount });
                else if (!factionId.empty())
                    stolenItemMutations.push_back({ added.refId, factionId, true, line.requestedCount });
            }
        }
    }
    normalizeContainerItems(resultingSource.items);
    normalizeContainerItems(removedSourceItems);

    result.accepted = true;
    result.error = InventoryTakeError::None;
    result.inventoryRevision = request.expectedInventoryRevision + 1;
    result.theft = theft;
    result.crimeValue = crimeValue;

    std::optional<CrimeSemanticService::Outcome> preparedCrime;
    if (theft && mObservationService)
    {
        CrimeWitnessBuildRequest witnessRequest;
        witnessRequest.eventCell = c.player.cell;
        witnessRequest.offender = makeLiveObservationSnapshot(*acceptedPlayer, playerBootWeight(c));
        ...
        CrimeIntent intent;
        intent.eventId = "inventory-take-batch:" + std::to_string(c.dbAccountId) + ":"
            + std::to_string(c.dbCharacterId) + ":" + request.requestId;
        intent.source = "authoritative_container_take_batch";
        intent.type = CrimeType::Theft;
        intent.cellId = canonicalPlayerCell;
        intent.offender = witnessRequest.offender;
        intent.victim = witnessRequest.victim;
        intent.value = crimeValue;
        intent.observedAtMs = nowMs;
        intent.maximumSnapshotAgeMs = MaximumSnapshotAgeMs;
        for (const std::string& cellId : witnesses.candidateCellIds)
        {
    ...
    InventoryTakeResult persistedResult;
    persistedResult.requestId = request.requestId;
    persistedResult.accepted = true;
    persistedResult.error = InventoryTakeError::None;
    persistedResult.kind = request.kind;
    persistedResult.source = request.source;
    persistedResult.itemRefId = "__inventory_take_batch__";
    persistedResult.itemCount = static_cast<std::int32_t>(request.items.size());
    persistedResult.inventoryRevision = result.inventoryRevision;
    persistedResult.theft = theft;
    persistedResult.crimeValue = crimeValue;
    ...
    commit.requestId = request.requestId;
    commit.requestHash = requestHash;
    commit.result = persistedResult;
    commit.expectedInventoryRevision = request.expectedInventoryRevision;
    commit.resultingInventoryRevision = result.inventoryRevision;
    commit.inventory = inventory;
    commit.expectedSource = expectedSource;
    commit.resultingSource = resultingSource;
    commit.stolenItemMutations = std::move(stolenItemMutations);
    if (preparedCrime && preparedCrime->pendingCommit)
        commit.crimeMutation = *preparedCrime->pendingCommit;
    ...
    {
        Log(Debug::Error) << "[InventoryTakeBatch] persistence failure request=" << request.requestId
                          << " player=" << c.name << " error=" << e.what();
        reject(InventoryTakeError::PersistenceFailure);
        return;
    }
    ...
    if (!result.replayed)
    {
        ...
        c.player.inventoryChanges.items = std::move(inventory);
        c.inventoryRevision = result.inventoryRevision;
        c.player.inventoryChanges.revision = result.inventoryRevision;
        c.restoredInventorySnapshot = c.player.inventoryChanges.items;
        c.hasRestoredInventorySnapshot = true;
        ...
        sourcePacket.container.mpNum = sourceIt->second.mpNum;
        sourcePacket.container.hasAuthority = true;
        sourcePacket.container.items = std::move(removedSourceItems);
        sourcePacket.authorityGeneration = currentActorAuthorityGenerationForCell(sourceIt->second.cellId);
        sourcePacket.mAction = static_cast<std::uint8_t>(ContainerAction::Remove);
        broadcastToCell(canonicalPlayerCell, sourcePacket.encode());
        scheduleGeneratedDynamicRecordGc("inventory_take_batch");
        if (haveSoundItem)
        {
            broadcastInventoryTransferSound(c,
                "inventory-take-batch:" + std::to_string(c.guid) + ":" + request.requestId,
                soundItem.refId, soundItem.instanceId, static_cast<std::int32_t>(result.itemCount),
                result.inventoryRevision, InventoryTransferMutation::Added, request.soundDirection);
        }
        syncLuaPlayerSnapshot();
    }
    sendAuthoritativeInventory(c);
    ...
    Log(Debug::Info) << "[InventoryTakeBatch] accepted request=" << request.requestId
                     << " source=" << request.source.refId
...
// ---------------------------------------------------------------------------
bool MPServer::ensureContainerItemIdentities(ContainerRecord& record)
{
    if (std::none_of(record.items.begin(), record.items.end(),
            [](const ContainerItem& item) { return item.count > 0 && item.instanceId == 0; }))
        return true;
    ContainerRecord assigned = record;
    if (!assignAuthoritativeContainerIdentities(assigned.items, [this] { return reserveWorldMpNum(); }))
        return false;
    if (mPlayerDb)
        mPlayerDb->upsertContainerRecord(assigned);
    record = std::move(assigned);
    return true;
}
...
void MPServer::handleContainer(ConnectedClient& c, const uint8_t* data, size_t size)
{
    ...
    if (action == ContainerAction::BootstrapRequest)
    {
        ...
        {
            if (!ensureContainerItemIdentities(currentIt->second))
                return;
    ...
    if (!senderAuthoritative)
    {
        if (action == ContainerAction::Set)
        {
            ...
            if (currentIt != mWorld.containers.end() && currentIt->second.hasAuthority)
            {
                ...
                current.authorityGeneration = currentAuthorityGeneration;
                current.mAction = static_cast<std::uint8_t>(ContainerAction::Set);
                current.bootstrapSequence = pkt.bootstrapSequence;
                sendTo(c.conn, current.encode());
            }
        }
    ...
    if (action == ContainerAction::Set)
    {
        ...
        {
            if (!ensureContainerItemIdentities(authoritative))
                return;
        ...
        ContainerRecord initial = pkt.container;
        initial.hasAuthority = true;
        // Initial client rows have local RefNums, not server-issued item IDs.
        // Allocate before normalization so separate scripted/damaged rows survive.
        for (auto& item : initial.items)
            item.instanceId = 0;
        if (!ensureContainerItemIdentities(initial))
            return;
        authoritative = std::move(initial);
        if (repairContainerRestockingMetadata(authoritative, std::max(1, c.player.level)))
        ...
        accepted.bootstrapSequence = pkt.bootstrapSequence;
        sendTo(c.conn, accepted.encode());
        accepted.bootstrapSequence = 0;
        ...
        Log(Debug::Info) << "[Server] Container(Set accepted): player=" << c.name
                         ...
                         << " mpNum=" << authoritative.mpNum
                         << " items=" << authoritative.items.size();
        for (const auto& item : authoritative.items)
            Log(Debug::Info) << "[InventoryTake] Container(Set) identity source=" << authoritative.refId
                << " mpNum=" << authoritative.mpNum << " refId=" << item.refId
                << " instanceId=" << item.instanceId << " count=" << item.count
                << " charge=" << item.charge << " enchant=" << item.enchantmentCharge
                << " soul=" << item.soul << " restocking=" << item.restocking;
        return;
    }
