...
namespace
{
    ...
    constexpr const char* BootstrapDeathAppliedUserValue = "mp_bootstrap_death_applied";

    uint64_t actorSyncLogClockMs()
    {
    ...
    bool restoreVanillaBaseAiPackages(
        const std::string& cellId, const MWWorld::Ptr& ptr, const mwmp::BaseActor& actor)
    {
        if (actor.mpNum != 0 || ptr.isEmpty() || !ptr.getClass().isActor())
            return false;
        ...
        if (ptr.getType() == ESM::NPC::sRecordId)
            aiSequence.fill(ptr.get<ESM::NPC>()->mBase->mAiPackage);
        else if (ptr.getType() == ESM::Creature::sRecordId)
            aiSequence.fill(ptr.get<ESM::Creature>()->mBase->mAiPackage);
        else
            return false;
        ...
        Log(Debug::Verbose) << "[MP] ActorSync: restored vanilla base AI packages after authority transfer"
                            << " cell=" << cellId
                            << " refId=" << actor.refId
                            << " refNum=" << actor.refNum;
        return true;
    ...
    void clearSyncedIdleAuthorityHandoffNode(
        const std::string& cellId, const MWWorld::Ptr& ptr, const mwmp::BaseActor& actor, const char* reason)
    {
        if (ptr.isEmpty())
            return;
        ...
        const bool hadSyncedIdle = baseNode->getUserValue("mp_synced_idle_group", syncedIdleGroup)
            && !syncedIdleGroup.empty();

        baseNode->setUserValue("mp_hold_synced_idle_after_authority_transfer", false);
        ...
        baseNode->setUserValue("mp_synced_idle_revision", 0.0);
        baseNode->setUserValue("mp_synced_idle_applied_revision", 0.0);
        baseNode->setUserValue("mp_synced_idle_applied_start_point", -1.f);
        baseNode->setUserValue("mp_current_idle_group", std::string());

        if (hadSyncedIdle)
        {
            Log(Debug::Verbose) << "[MP] ActorSync: cleared synced idle authority handoff"
                                << " cell=" << cellId
                                << " refId=" << actor.refId
                                << " refNum=" << actor.refNum
                                << " mpNum=" << actor.mpNum
                                << " group='" << syncedIdleGroup << "'"
                                << " reason=" << reason;
        }
    }

    bool restoreAuthorityActorAiIfNeeded(const std::string& cellId, const MWWorld::Ptr& ptr,
        const mwmp::BaseActor& actor, const ESM::AiSequence::AiSequence* savedPuppetAiSequence)
    {
        ...
        bool preservedSyncedIdleHandoff = false;
        if (auto* baseNode = ptr.getRefData().getBaseNode())
        {
            ...
            std::string syncedIdleGroup;
            float syncedIdleStartPoint = -1.f;
            const bool hasSyncedIdle = baseNode->getUserValue("mp_synced_idle_group", syncedIdleGroup)
                && !syncedIdleGroup.empty()
                && baseNode->getUserValue("mp_synced_idle_start_point", syncedIdleStartPoint)
                && syncedIdleStartPoint >= 0.f;

            if (!wasRemotePuppet && !hadSavedAi)
                return false;

            baseNode->setUserValue("mp_remote_actor", false);
            // mp_interp_speed is a puppet-only CharacterController override.
            // Leaving the observer's final interpolation velocity on the node
            // after an authority handoff makes the newly authoritative actor's
            // real physics and animation run at that stale (often very low)
            // speed.
            baseNode->setUserValue("mp_interp_speed", 0.f);
            if (hasSyncedIdle && wasRemotePuppet)
            {
                baseNode->setUserValue("mp_hold_synced_idle_after_authority_transfer", true);
                preservedSyncedIdleHandoff = true;
                Log(Debug::Verbose) << "[MP] ActorSync: preserved synced idle during authority handoff"
                                    << " cell=" << cellId
                                    << " refId=" << actor.refId
                                    << " refNum=" << actor.refNum
                                    << " mpNum=" << actor.mpNum
            ...
            {
                clearSyncedIdleAuthorityHandoffNode(cellId, ptr, actor, "authority-restore");
            }
        ...
        MWMechanics::AiSequence& aiSequence = stats.getAiSequence();
        const bool packetRequestsCombat = actor.ai.type == mwmp::BaseActor::AIAction::Type::Combat;
        if (packetRequestsCombat && !aiSequence.isInCombat())
        {
            MWWorld::Ptr combatTarget;
            MWBase::World* combatWorld = MWBase::Environment::get().getWorld();
            const std::string& targetId = actor.ai.targetId;
            if (combatWorld && !targetId.empty())
            {
                ...
                if (targetId.rfind(remotePrefix, 0) == 0)
                {
                    ...
                    for (char ch : targetId.substr(remotePrefix.size()))
                    {
                        ...
                        targetGuid = targetGuid * 10u + static_cast<uint32_t>(ch - '0');
                    }
                    if (parsedGuid && targetGuid != 0)
                    {
                        if (targetGuid == mwmp::Main::get().getPlayerSync().localPlayer().guid)
                            combatTarget = combatWorld->getPlayerPtr();
                        else if (auto* remotePlayer = mwmp::Main::get().getPlayerList().getPlayer(targetGuid))
                            combatTarget = remotePlayer->getNpcPtr();
                    }
                }
                ...
                if (combatTarget.isEmpty())
                {
                    ...
                    if (!localPlayer.isEmpty()
                        && localPlayer.getCellRef().getRefId().serializeText() == targetId)
                        combatTarget = localPlayer;
                }
            }
            ...
            {
                aiSequence.clear();
                aiSequence.stack(MWMechanics::AiCombat(combatTarget), ptr);
                int fight = stats.getAiSetting(MWMechanics::AiSetting::Fight).getBase();
                if (fight < 30)
                {
                    fight = 30;
                ...
                return preservedSyncedIdleHandoff;
            }
        }
        ...
        if (savedPuppetAiSequence && !savedPuppetAiSequence->mPackages.empty())
        {
            aiSequence.readState(*savedPuppetAiSequence);
            Log(Debug::Verbose) << "[MP] ActorSync: restored saved AI packages after authority transfer"
                                << " cell=" << cellId
                                << " refId=" << actor.refId
                                << " refNum=" << actor.refNum
                                << " mpNum=" << actor.mpNum
                                << " packages=" << savedPuppetAiSequence->mPackages.size();
            return preservedSyncedIdleHandoff;
        }

        // Do not invent a fallback wander when a local content actor is promoted
        // from remote puppet to authority.  Restore its authored base package
        // stack instead; this keeps package-driven idle actors such as Suran
        // dancers alive after handoff without converting them to generic wanderers.
        if (actor.mpNum == 0)
        {
            if (!restoreVanillaBaseAiPackages(cellId, ptr, actor))
            {
                Log(Debug::Verbose) << "[MP] ActorSync: no vanilla AI packages to restore after authority transfer"
                                    << " cell=" << cellId
                                    << " refId=" << actor.refId
                                    << " refNum=" << actor.refNum;
            }
    ...
    std::string activePresentationAnimGroup(const MWWorld::Ptr& ptr, MWRender::Animation& animObj)
    {
        if (!ptr.isEmpty())
        {
            ...
            if (const auto* baseNode = ptr.getRefData().getBaseNode();
                ...
                && animObj.getInfo(currentIdleGroup))
            {
        ...
        for (MWRender::BoneGroup boneGroup : kUpperBodyGroups)
        {
            ...
            if (isReliablePresentationAnimGroup(group))
                return group;
        }
    ...
    bool isWatchedPresentationActor(const mwmp::BaseActor& actor, mwmp::ActorInstanceId actorNetId)
    {
        ...
        return actor.refId == "heddvild" || actor.mpNum == 2601
            ...
            || actor.refId == "tr_m3_bribanne erien"
            || actor.refId == "tr_m3_valkreia krex"
            || actor.refId == "tr_m3_arquebald vene"
...
namespace mwmp
{
    ...
    bool isRandomCreatureAttackGroup(const std::string& group)
    {
        return group == "attack1" || group == "attack2" || group == "attack3"
            || group == "swimattack1" || group == "swimattack2" || group == "swimattack3";
    }

    ActorInstanceId ActorSync::actorNetIdForPtr(
        const std::string& cellId, const MWWorld::Ptr& ptr) const
    {
        if (ptr.isEmpty() || !ptr.getClass().isActor())
            return 0;

        const uint32_t mappedMpNum = mappedMpNumForPtr(cellId, ptr);
        if (mappedMpNum != 0)
            return packActorInstanceKey({ ActorKeyKind::SpawnedMpNum, mappedMpNum });

        // Reconciled leveled-list children have a client-local dynamic refNum.
        // Their network identity remains the placed spawner's vanilla refNum.
        for (const auto& [actorNetId, reconciledPtr] : mReconciledVanillaActorsByNetId)
        {
            if (!reconciledPtr.isEmpty()
                && (reconciledPtr == ptr || sameLocalActorObject(reconciledPtr, ptr)))
                return actorNetId;
        }

        // The primary runtime can already be bound before the reconciliation
        // map is populated. Preserve that canonical identity during the brief
        // handoff window as well.
    ...
    void ActorSync::update(float dt)
    {
        ...
        for (auto& [actorNetId, actor] : mActorsByNetId)
        {
            ...
            actor.cellChangeReverseGuardTimer
                = std::max(0.f, actor.cellChangeReverseGuardTimer - std::max(0.f, dt));
            if (!actor.pendingCellChangeDestination.empty()
                && actor.state.cellId == actor.pendingCellChangeDestination)
            {
                actor.pendingCellChangeDestination.clear();
                actor.pendingCellChangeRetryTimer = 0.f;
            }

            if (actor.state.cellId.empty())
                continue;

            if (hasAuthorityForActor(actorNetId, actor.state.cellId))
            {
                const std::string boundCellId = actor.boundActor.isEmpty()
                    ? std::string() : cellIdForPtr(actor.boundActor);
                ...
                // A committed ActorCellChange updates the canonical v2 runtime
                // before the destination authority's existing MWWorld::Ptr is
                // physically moved out of its source CellStore. Treat that as
                ...
                if (!actor.boundActor.isEmpty())
                {
                    ...
                    {
                        bool wasAuthoritativeActor = false;
                        const bool hadAuthorityFlag
                            = baseNode->getUserValue("mp_authoritative_actor", wasAuthoritativeActor);
                        baseNode->setUserValue("mp_authoritative_actor", true);
                        baseNode->setUserValue("mp_remote_actor", false);
                        baseNode->setUserValue("mp_interp_speed", 0.f);
                        if (!hadAuthorityFlag || !wasAuthoritativeActor)
                        {
                            Log(Debug::Info) << "[MP] ActorSync: enabled authoritative actor simulation"
                                             << " cell=" << actor.state.cellId
                                             << " refId=" << actor.state.refId
                                             << " mpNum=" << actor.state.mpNum
                                             << " boundCell=" << cellIdForPtr(actor.boundActor);
                        }
                    }
                    restoreAuthorityActorAiIfNeeded(actor.state.cellId, actor.boundActor,
                        actor.state, actor.savedPuppetAiSequence.get());
                }
                continue;
            }
        ...
        {
            // ActorDeath baselines can arrive before ActorIdentity promotes a
            // fallback cell runtime into mActorsByNetId. On the cell authority,
            // apply those dead baselines before scanning the local CellStore.
            // Otherwise the first authoritative scan advertises the content
            // actor as alive, mechanics gives it a live AI tick, and the later
            // correction is incorrectly replayed as a real-time death.
            for (auto& [actorKey, actor] : cell.actors)
            {
                if (!actor.state.isDead || !actor.hasAuthoritativeTransform)
                    continue;
            ...
            const bool hasCellAuthority = hasAuthority(cellId);
            bool hasLeasedActorAuthority = false;
            if (!hasCellAuthority)
            {
                for (const auto& [actorNetId, runtime] : mActorsByNetId)
                {
                    if (runtime.state.cellId == cellId
                        && hasAuthorityForActor(actorNetId, cellId))
                    {
                        hasLeasedActorAuthority = true;
                        break;
                    }
                }
            }
            if (hasCellAuthority || hasLeasedActorAuthority)
    ...
    void ActorSync::updateLoadedCellBootstrapVisuals()
    {
        ...
        auto applyPendingCorpse = [&](const std::string& cellId, ActorRuntime& actor)
        {
            ...
            {
                if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
                    baseNode->getUserValue(BootstrapDeathAppliedUserValue, appliedToBinding);
            }
            if (appliedToBinding)
                return;

            // A cell can be inserted by StateManager/World after the regular
            // multiplayer update at the start of this engine frame. Resolve
            // only pending corpses here so their bind pose cannot reach this
            // frame's render traversal.
            if (resolveActorBinding(cellId, actor))
            {
                applyBootstrapDeathState(actor);
                if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
                {
                    bool nowApplied = false;
                    if (baseNode->getUserValue(BootstrapDeathAppliedUserValue, nowApplied) && nowApplied)
                        ++bootstrappedAfterWorldUpdate;
                }
            }
        };
        ...
        {
            if (!actor.state.cellId.empty())
                applyPendingCorpse(actor.state.cellId, actor);
        }
        ...
        for (auto& [cellId, cell] : mCells)
        {
            for (auto& [actorKey, actor] : cell.actors)
            {
                if (actor.actorNetId != 0
                    && mActorsByNetId.find(actor.actorNetId) != mActorsByNetId.end())
                    continue;
                applyPendingCorpse(cellId, actor);
            }
        }

        if (bootstrappedAfterWorldUpdate != 0)
        {
    ...
    {
        // Actor-bearing authority packets are per-actor lease updates. They do
        // not change the fallback authority for the containing cell.
        if (!list.actors.empty())
        {
            for (const BaseActor& actor : list.actors)
            {
                const ActorInstanceId actorNetId = actorNetIdForActorState(actor);
                if (actorNetId == 0)
                    continue;
                ...
                Log(Debug::Verbose) << "[MP] ActorSync: actor authority lease"
                                 ...
                                 << " generation=" << list.authorityGeneration;
            }
            return;
        }
        ...
        if (previous != hasLocalAuthority)
        {
            ...
            if (!previous && hasLocalAuthority)
            {
                ...
                {
                    runtime.lastSentPresentationValid = false;
                    runtime.lastSentAnimGroup.clear();
                    runtime.lastSentPresentationFlags = 0;
                    runtime.presentationSendTimer = 0.f;
                    runtime.authorityPendingReliableAnimGroup.clear();
                    runtime.authorityPendingReliableAnimGroupTimer = 0.f;
                    runtime.authorityReliableAnimGroupLostTimer = 0.f;
                    ++presentationReset;
                };

                for (auto& [actorKey, runtime] : cell.actors)
                    resetAuthorityPresentation(runtime);
                ...
                Log(Debug::Verbose) << "[MP] ActorSync: authority grant reset presentation baseline"
                                    << " cell=" << list.cellId
                                    << " actors=" << presentationReset
                                    << " gen=" << list.authorityGeneration;
            }
            else if (previous && !hasLocalAuthority)
            {
                ...
                auto markRuntimeForRebase = [&](ActorRuntime& runtime)
                {
                    ...
                    runtime.lastPresentationServerTimestamp = 0;
                    runtime.syncedSpecialIdleClearTimer = 0.f;
                    runtime.syncedIdleRevision = 0;
                    runtime.lastAppliedAnimGroup.clear();
                    runtime.animGroupHoldTimer = 0.f;
                    if (!runtime.boundActor.isEmpty())
                    {
    ...
    void ActorSync::onActorIdentityUpdate(const ActorIdentityList& list)
    {
        ...
        for (const ActorIdentityRecord& record : list.actors)
        {
            ...
            if (!record.removed && actorState.mpNum == 0 && actorState.refNum != 0)
                authoritativeVanillaRefNums.insert(actorState.refNum);
            ...
            // Never downgrade the runtime to an older generation.
            // A newer identity record is authoritative recovery.
            if (generationIsNewer(record.migrationGeneration, runtime.migrationGeneration))
            {
                runtime.migrationGeneration = record.migrationGeneration;

                // Discard pending snapshots older than the identity generation.
                for (auto it = runtime.pendingSnapshotsByMigrationGeneration.begin();
                     it != runtime.pendingSnapshotsByMigrationGeneration.end();)
                {
                    if (generationIsNewer(runtime.migrationGeneration, it->first))
                        it = runtime.pendingSnapshotsByMigrationGeneration.erase(it);
                    else
                        ++it;
                }
            }
            else if (runtime.migrationGeneration == 0)
            {
                // Initial assignment from zero.
                runtime.migrationGeneration = record.migrationGeneration;
            }

            // Keep the duplicated BaseActor generation in sync with the runtime.
            runtime.state.migrationGeneration = runtime.migrationGeneration;
        ...
        // actors in the cell. A placed leveled-list spawner absent from that set
        ...
        if (completeIdentitySnapshot)
        {
            ...
            if (world)
            {
                ...
                if (MWWorld::CellStore* activeCell = findActiveCellById(worldImp, list.cellId))
                {
                    auto& chanceNoneSpawners = mChanceNoneLeveledSpawnersByCell[list.cellId];
                    std::size_t chanceNoneRetired = 0;
                    activeCell->forEach([&](MWWorld::Ptr ptr) -> bool
                    {
                        ...
                        // Absence from a complete identity snapshot is the
                        // authoritative representation of Chance None. Record
                        // it even when this particular load has not rolled a
                        // child, otherwise a later cell reload or authority
                        // handoff can create a client-only creature.
                        chanceNoneSpawners.insert(actorNetId);
                    ...
                    if (chanceNoneRetired != 0)
                    {
                        Log(Debug::Info) << "[MP] ActorSync: retired non-authoritative leveled rolls"
                                         << " cell=" << list.cellId
                                         << " count=" << chanceNoneRetired
                                         << " identityActors=" << list.actors.size();
                    }
                }
            }
        }
    ...
    void ActorSync::onActorListUpdate(const ActorList& list)
    {
        ...
        for (const auto& actorState : list.actors)
        {
            ...
            if (actorNetId != 0 && !hasAuthorityForActor(actorNetId, list.cellId))
            {
    ...
    void ActorSync::onActorCellChange(const ActorList& list)
    {
        ...
        for (const auto& actorState : list.actors)
        {
            ...
            {
                // Generation check: ignore stale commits, skip duplicates.
                if (primaryRuntime->migrationGeneration != 0
                    && generationIsNewer(primaryRuntime->migrationGeneration, actorState.migrationGeneration))
                {
                    Log(Debug::Verbose) << "[MP] ActorSync: dropped stale ActorCellChange"
                                        << " actorNetId=" << primaryRuntime->actorNetId
                                        << " commitGen=" << actorState.migrationGeneration
                                        << " runtimeGen=" << primaryRuntime->migrationGeneration;
                    continue;
                }
                if (primaryRuntime->migrationGeneration == actorState.migrationGeneration
                    && actorState.migrationGeneration != 0)
                {
                    Log(Debug::Verbose) << "[MP] ActorSync: duplicate ActorCellChange ignored"
                                        << " actorNetId=" << primaryRuntime->actorNetId
                                        << " gen=" << actorState.migrationGeneration;
                    continue;
                }

                const std::string oldCellId = primaryRuntime->state.cellId;
                primaryRuntime->migrationGeneration = actorState.migrationGeneration;

                // Discard pending snapshots from older generations first.
                for (auto it = primaryRuntime->pendingSnapshotsByMigrationGeneration.begin();
                     it != primaryRuntime->pendingSnapshotsByMigrationGeneration.end();)
                {
                    if (generationIsNewer(primaryRuntime->migrationGeneration, it->first))
                        it = primaryRuntime->pendingSnapshotsByMigrationGeneration.erase(it);
                    else
                        ++it;
                }

                // Collect and sort pending snapshots for the committed generation
                // by serverTimestamp (then sequence) so the interpolation timeline
                // never runs backwards.
                std::vector<BufferedSnapshot> sortedPending;
                auto pendingIt = primaryRuntime->pendingSnapshotsByMigrationGeneration.find(
                    primaryRuntime->migrationGeneration);
                if (pendingIt != primaryRuntime->pendingSnapshotsByMigrationGeneration.end())
                {
                    sortedPending.assign(pendingIt->second.begin(), pendingIt->second.end());
                    primaryRuntime->pendingSnapshotsByMigrationGeneration.erase(pendingIt);
                    std::sort(sortedPending.begin(), sortedPending.end(),
                        [](const BufferedSnapshot& a, const BufferedSnapshot& b)
                        {
                ...
                // Choose the best current visual transform for the CellStore move.
                // Prefer the current smoothed position, then the latest pending
                // snapshot for the committed generation, then the commit transform.
                Position cellMovePosition = actorState.position;
                if (!sortedPending.empty())
                    cellMovePosition = sortedPending.back().position;
                if (primaryRuntime->hasSmoothedPosition)
                    cellMovePosition = primaryRuntime->smoothedPosition;

                if (oldCellId != destCellId)
                {
                    ...
                    // Remove legacy source-cell copy.
                    auto oldCellIt = mCells.find(oldCellId);
                    if (oldCellIt != mCells.end())
                    {
                        for (auto it = oldCellIt->second.actors.begin(); it != oldCellIt->second.actors.end();)
                        {
                            const BaseActor& candidate = it->second.state;
                            const bool sameActor = actorState.mpNum != 0
                                ? candidate.mpNum == actorState.mpNum
                                : candidate.mpNum == 0 && candidate.refNum == actorState.refNum
                                    && candidate.refId == actorState.refId;
                            if (sameActor)
                                it = oldCellIt->second.actors.erase(it);
                            else
                                ++it;
                        }
                    }
                ...
                if (!destinationAuthority && handoffState.isMoving)
                {
                    const float velocityX = handoffState.velocity.linear[0];
                    const float velocityY = handoffState.velocity.linear[1];
                    const float planarSpeed = std::sqrt(
                        velocityX * velocityX + velocityY * velocityY);
                    constexpr float kMaxHandoffPredictionSpeed = 200.f;
                    if (planarSpeed > kMaxHandoffPredictionSpeed)
                    {
                        const float velocityScale = kMaxHandoffPredictionSpeed / planarSpeed;
                        for (float& component : handoffState.velocity.linear)
                            component *= velocityScale;
                    }
                }
                // Keep the active visual timeline generation-consistent. For a
                // seamless adjacent crossing, the retained snapshots are local
                // interpolation history, so retag them to the committed generation.
                // For discontinuities, discard every superseded-generation sample.
                auto& activeSnapshots = primaryRuntime->snapshots;
                if (preserveBufferedTimeline)
                {
                    for (BufferedSnapshot& snapshot : activeSnapshots)
                        snapshot.migrationGeneration = primaryRuntime->migrationGeneration;
                }
                else
                {
                    activeSnapshots.erase(
                        std::remove_if(
                            activeSnapshots.begin(),
                            activeSnapshots.end(),
                            [&](const BufferedSnapshot& snapshot)
                            {
                                return snapshot.migrationGeneration
                                    != primaryRuntime->migrationGeneration;
                            }),
                        activeSnapshots.end());
                }

                // Synchronize the duplicated generation in BaseActor so every
                // subsequent compact snapshot and reliable state packet uses the
                // canonical committed generation.
                primaryRuntime->state.migrationGeneration
                    = primaryRuntime->migrationGeneration;
                handoffState.migrationGeneration = primaryRuntime->migrationGeneration;
                ...
                if (bindingResolved)
                {
                    ...
                    // has already entered the destination CellStore. Rebase it
                        ...
                        && !primaryRuntime->state.isDead)
                    {
                        ...
                        if (world)
                        {
                            MWWorld::CellStore* destinationCell = findActiveCellById(
                                static_cast<MWWorld::World&>(*world), destCellId);
                            if (destinationCell)
                            {
                                const osg::Vec3f movePos(cellMovePosition.pos[0],
                                    cellMovePosition.pos[1], cellMovePosition.pos[2]);
                                primaryRuntime->boundActor = world->moveObject(
                                    primaryRuntime->boundActor, destinationCell, movePos);
                            }
                        }
                    }

                    if (destinationAuthority && !primaryRuntime->state.isDead)
                    {
                        primaryRuntime->rebaseOnNextAuthoritativeSnapshot = true;
                        MWBase::World* world = MWBase::Environment::get().getWorld();
                        if (world && !primaryRuntime->boundActor.isEmpty())
                        {
                            MWMechanics::Movement& movement
                                = primaryRuntime->boundActor.getClass().getMovementSettings(
                                    primaryRuntime->boundActor);
                            float handoffSide = primaryRuntime->state.animFlags.animSide;
                            float handoffFwd = primaryRuntime->state.animFlags.animFwd;
                            const bool handoffHasAiPackage
                                = primaryRuntime->state.ai.type != BaseActor::AIAction::Type::None;
                            const bool handoffHasAxes = std::abs(handoffFwd) > 0.1f
                                || std::abs(handoffSide) > 0.1f;
                            const bool handoffShouldMove = primaryRuntime->state.isMoving
                                || (!primaryRuntime->state.isDead && handoffHasAiPackage);
                            if (handoffShouldMove && !handoffHasAxes)
                            {
                                handoffFwd = 1.f;
                                handoffSide = 0.f;
                            }
                            movement.mPosition[0] = handoffShouldMove ? handoffSide : 0.f;
                            movement.mPosition[1] = handoffShouldMove ? handoffFwd : 0.f;

                            Log(Debug::Info) << "[MP] ActorSync: rebased destination authority at cell commit"
                                             << " cell=" << destCellId
                                             << " refId=" << actorState.refId
                                             << " mpNum=" << actorState.mpNum
                                             << " migrationGen=" << primaryRuntime->migrationGeneration;
                        }
                    }
                    // Do NOT call applyBoundActorState here for observers; the
                    // explicit-cell move above already placed the actor in the
                    // destination CellStore. applyBoundActorState uses the
                    // coordinate-derived overload which can move the Ptr back
                    // into the source cell when the interpolated position has
                    // not yet crossed the border.
                    if (hasAuthorityForActor(primaryRuntime->actorNetId, destCellId))
                        applyBoundActorState(*primaryRuntime);
                }

                // Replay sorted pending snapshots for the committed generation.
                for (const BufferedSnapshot& pending : sortedPending)
                {
                    BaseActor pendingState = primaryRuntime->state;
                    pendingState.position = pending.position;
                    pendingState.velocity = pending.velocity;
                    pendingState.isMoving = pending.isMoving;
                    pendingState.isAttackingOrCasting = pending.isAttackingOrCasting;
                    pendingState.hasWeaponDrawn = pending.hasWeaponDrawn;
                    pendingState.hasSpellReadied = pending.hasSpellReadied;
                    pendingState.animFlags.movementFlags = pending.movementFlags;
                    pendingState.animFlags.animFwd = pending.animFwd;
                    pendingState.animFlags.animSide = pending.animSide;
                    pendingState.migrationGeneration = primaryRuntime->migrationGeneration;

                    ActorList synthetic;
                    synthetic.cellId = destCellId;
                    synthetic.snapshotSequence = pending.sequence;
                    synthetic.serverTimestamp = pending.serverTimestamp;
                    queueSnapshot(*primaryRuntime, pendingState, synthetic);
                }

                // PositionV2 can overtake the reliable cell commit. Replay the
                // buffered snapshots for this generation after canonical binding,
                // preserving their server timestamp order.
                for (const BufferedSnapshot& pending : sortedPending)
                {
                    BaseActor pendingState = primaryRuntime->state;
                    pendingState.position = pending.position;
            ...
            const std::string oldCellId = runtime.state.cellId.empty() ? list.cellId : runtime.state.cellId;
            if (oldCellId != destCellId)
            {
                runtime.previousCellChangeCellId = oldCellId;
                runtime.cellChangeReverseGuardTimer = 5.f;
                runtime.cellChangeReverseGuardLogged = false;
            }

            auto& destCell = mCells[destCellId];
            runtime.state = actorState;
            runtime.migrationGeneration = actorState.migrationGeneration;

            // Discard older-generation pending snapshots.
            for (auto it = runtime.pendingSnapshotsByMigrationGeneration.begin();
                 it != runtime.pendingSnapshotsByMigrationGeneration.end();)
            {
                if (generationIsNewer(runtime.migrationGeneration, it->first))
    ...
    void ActorSync::onActorPositionV2Update(const ActorPositionV2List& list)
    {
        ...
        for (const CompactActorSnapshot& snapshot : list.snapshots)
        {
            ...
            // Generation zero is a valid initial generation. Compare
            // unconditionally so the first crossing (gen 0 → gen 1) is
            // buffered correctly rather than applied before its commit.
            if (generationIsNewer(runtime.migrationGeneration, snapshot.migrationGeneration))
            {
                // Snapshot belongs to a source-cell timeline that has been superseded.
                ++stale;
                continue;
            }
            if (generationIsNewer(snapshot.migrationGeneration, runtime.migrationGeneration))
            {
                // Snapshot is from a future migration whose reliable commit has not arrived.
                // Buffer it; do not apply it or use it to infer cell ownership.
                // Check the map size before insertion to avoid unbounded growth from
                // operator[] side effects.
                auto pendingIt = runtime.pendingSnapshotsByMigrationGeneration.find(
                    snapshot.migrationGeneration);
                if (pendingIt == runtime.pendingSnapshotsByMigrationGeneration.end())
                {
                    if (runtime.pendingSnapshotsByMigrationGeneration.size() >= 2)
                        continue; // Drop silently — at most two future generations.
                    pendingIt = runtime.pendingSnapshotsByMigrationGeneration
                        .emplace(snapshot.migrationGeneration, std::deque<BufferedSnapshot>{})
                        .first;
                }
                auto& pendingQueue = pendingIt->second;
                if (pendingQueue.size() < 16)
                {
                    BufferedSnapshot pending;
                    pending.position = snapshot.position;
                    pending.velocity = snapshot.velocity;
                    pending.sequence = list.sequence;
                    pending.serverTimestamp = list.serverTimestamp;
                    pending.migrationGeneration = snapshot.migrationGeneration;
                    pending.isMoving = (snapshot.presentationFlags & ActorPresentationMoving) != 0;
                    pending.isAttackingOrCasting = (snapshot.presentationFlags & ActorPresentationAttackingOrCasting) != 0;
                    pending.hasWeaponDrawn = (snapshot.presentationFlags & ActorPresentationWeaponDrawn) != 0;
                    pending.hasSpellReadied = (snapshot.presentationFlags & ActorPresentationSpellReadied) != 0;
                    pending.movementFlags = snapshot.movementFlags;
                    pending.animFwd = dequantizeActorAxis(snapshot.animFwd);
                    pending.animSide = dequantizeActorAxis(snapshot.animSide);
                    pendingQueue.push_back(std::move(pending));
                    Log(Debug::Verbose) << "[MP] ActorSync: buffered future-generation position"
                                        << " actorNetId=" << snapshot.actorNetId
                                        << " snapshotGen=" << snapshot.migrationGeneration
                                        << " runtimeGen=" << runtime.migrationGeneration
                                        << " pendingQueues=" << runtime.pendingSnapshotsByMigrationGeneration.size();
                }
                continue;
            }
            ...
            const bool ownsProvisionalBoundCell = runtime.state.mpNum != 0
                ...
                && isExteriorActorCellId(boundCellId)
                && hasAuthorityForActor(runtime.actorNetId, boundCellId)
            ...
            // ActorDeath and the dead identity baseline are reliable and carry
            // the canonical death transform. Once that state has been applied,
            // continuing to consume unreliable dead position samples makes the
            // remote corpse fight its local settling/grounding every frame.
            // The visible result is a corpse repeatedly falling and snapping
            // back upward. Keep the one-time reliable placement instead.
            if (runtime.deathAlreadyApplied && snapshotIsDead)
            {
                ++deadLiveSuppressed;
                continue;
            }
            ...
            // The destination authority commonly emits one scan immediately
            // after ActorCellChange, before its first mechanics step. That scan
            // repeats the moving commit position with zero velocity. Accepting
            // it rewinds observers from commit extrapolation back to the border,
                    ...
                    runtime.state.mpNum, !runtime.previousCellChangeCellId.empty(), runtime.snapshots.size()))
            {
                ...
                const BufferedSnapshot& commitSnapshot = runtime.snapshots.back();
                const float deltaX = actorState.position.pos[0] - commitSnapshot.position.pos[0];
                const float deltaY = actorState.position.pos[1] - commitSnapshot.position.pos[1];
                const float deltaZ = actorState.position.pos[2] - commitSnapshot.position.pos[2];
                const float deltaSq = deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ;
                const float commitSpeedSq
                    = commitSnapshot.velocity.linear[0] * commitSnapshot.velocity.linear[0]
                    + commitSnapshot.velocity.linear[1] * commitSnapshot.velocity.linear[1]
                    + commitSnapshot.velocity.linear[2] * commitSnapshot.velocity.linear[2];
                const float incomingSpeedSq
                    = actorState.velocity.linear[0] * actorState.velocity.linear[0]
                    + actorState.velocity.linear[1] * actorState.velocity.linear[1]
                    + actorState.velocity.linear[2] * actorState.velocity.linear[2];
                constexpr float kCommitEchoPositionEpsilonSq = 4.f * 4.f;
                constexpr float kCommitMovingVelocitySq = 20.f * 20.f;
                constexpr float kCommitEchoVelocitySq = 5.f * 5.f;
                if (commitSnapshot.isMoving
                    && commitSpeedSq > kCommitMovingVelocitySq
                    && incomingSpeedSq < kCommitEchoVelocitySq
                    && deltaSq < kCommitEchoPositionEpsilonSq)
                {
                    Log(Debug::Info) << "[MP] ActorSync: suppressed stationary destination-authority echo"
                                     << " cell=" << runtime.state.cellId
                                     << " refId=" << runtime.state.refId
                                     << " mpNum=" << runtime.state.mpNum
                                     << " deltaSq=" << deltaSq
                                     << " commitSpeedSq=" << commitSpeedSq
                                     << " ts=" << list.serverTimestamp;
                    ++provisionalAuthoritySuppressed;
                    continue;
                }
            ...
            // ActorCellChange is the reliable commit for cell identity. Position
            // snapshots can overtake that commit on the transport and can be
            // older than more than one cell when fast actors are involved. Do
            // not let such a snapshot move either the Ptr or its transform back
            // outside the committed cell. A position clearly inside the
            ...
            if (runtime.state.mpNum != 0
                ...
                && !snapshotPositionCellId.empty())
            {
                constexpr float kCellChangeTransformSettledHysteresis = 64.f;
                const bool snapshotLandedInCanonicalCell = snapshotPositionCellId == runtime.state.cellId
                    && exteriorCellBorderDistance(actorState.position) > kCellChangeTransformSettledHysteresis;
                if (snapshotLandedInCanonicalCell)
                {
                    runtime.previousCellChangeCellId.clear();
                    runtime.cellChangeReverseGuardTimer = 0.f;
                    runtime.cellChangeReverseGuardLogged = false;
                }
                else if (runtime.cellChangeReverseGuardTimer > 0.f
                    && snapshotPositionCellId == runtime.previousCellChangeCellId)
                {
                    if (!runtime.cellChangeReverseGuardLogged)
                    {
                        runtime.cellChangeReverseGuardLogged = true;
                        Log(Debug::Info) << "[MP] ActorSync: suppressed stale reverse transform after ActorCellChange"
                                         << " canonical=" << runtime.state.cellId
                                         << " snapshotCell=" << snapshotPositionCellId
                                         << " previous=" << runtime.previousCellChangeCellId
                                         << " boundCell=" << boundCellId
                                         << " reverseGuard=" << runtime.cellChangeReverseGuardTimer
                                         << " refId=" << runtime.state.refId
                                         << " mpNum=" << runtime.state.mpNum
                                         << " ts=" << list.serverTimestamp;
                    }
    ...
    void ActorSync::onActorPresentationV2Update(const ActorPresentationV2List& list)
    {
        ...
        for (const ActorPresentationSnapshot& snapshot : list.snapshots)
        {
            ...
            const bool previousIsSpecialIdle = isIdleAnimGroup(previousGroup)
                && isReliablePresentationAnimGroup(previousGroup)
                && !isBaseIdleAnimGroup(previousGroup);
            ...
            if (!runtime.boundActor.isEmpty())
            {
                if (auto* baseNode = runtime.boundActor.getRefData().getBaseNode())
                {
                    std::string nodeSyncedIdleGroup;
                    double nodeSyncedIdleRevision = 0.0;
                    nodeHasSyncedIdleEvent = baseNode->getUserValue("mp_synced_idle_group", nodeSyncedIdleGroup)
                        && nodeSyncedIdleGroup == snapshot.currentAnimGroup
                        && baseNode->getUserValue("mp_synced_idle_revision", nodeSyncedIdleRevision)
                        && nodeSyncedIdleRevision > 0.0;
                }
            }
            // Same-group special-idle refreshes are clock samples, not replay events.
            // Keep the locally playing loop continuous; only a newly accepted idle event
            ...
            if (acceptIncomingSpecialIdle)
            {
                ...
                runtime.syncedIdleRevision = runtime.syncedIdleRevision == std::numeric_limits<uint32_t>::max()
                    ? 1
            ...
            if (!runtime.boundActor.isEmpty() && !locallyAuthoritative)
            {
                if (auto* baseNode = runtime.boundActor.getRefData().getBaseNode())
                {
                    if (acceptIncomingSpecialIdle)
                    {
                        ...
                        baseNode->setUserValue("mp_synced_idle_single_cycle", runtime.state.animFlags.idleSingleCycle);
                        ...
                        baseNode->setUserValue("mp_synced_idle_revision",
                            static_cast<double>(runtime.syncedIdleRevision));
                    }
            ...
            // Special idles are event-like: the authority chooses which idle starts,
            // and remotes should keep playing that idle until a real presentation state
            // break occurs.  Accept the first special idle, but ignore later special-idle
            // churn and transient empty/base-idle samples so the remote CharacterController
            ...
            else if (snapshot.currentAnimGroup == "idle")
            {
                // An explicit authority rest state ends the special idle without
                // handing lower-body selection back to displacement inference.
                runtime.state.animFlags.currentAnimGroup = snapshot.currentAnimGroup;
                runtime.state.animFlags.currentAnimCompletion = -1.f;
            }
            ...
            if (isWatchedPresentationActor(runtime.state, snapshot.actorNetId))
            {
                Log(Debug::Info) << "[MPWATCH] ActorSync v2: presentation apply"
                                    ...
                                    << " stateCompletion=" << runtime.state.animFlags.currentAnimCompletion
                                    << " snapshotGroup='" << snapshot.currentAnimGroup << "'"
    ...
    {
        auto& cell = mCells[list.cellId];
        // ActorDeath is a reliable event, not replaceable cell state.  A newer
        // stats, AI, authority, or presentation packet can legitimately be
        // processed first, especially when the server is relaying a busy cell.
        // Rejecting the death by the cell-wide snapshot sequence loses the only
        // packet that carries the canonical death animation.  Per-actor
        // deathEventId below is the ordering/deduplication authority for this
        // event stream.
        if (list.snapshotSequence != 0
            && cell.latestReliableSequence != 0
            && list.snapshotSequence < cell.latestReliableSequence)
        {
            Log(Debug::Verbose) << "[MP] ActorSync: accepting out-of-order ActorDeath event"
                                << " cellId=" << list.cellId
                                << " seq=" << list.snapshotSequence
                                << " latestReliableSeq=" << cell.latestReliableSequence;
        }
        cell.latest.snapshotSequence = std::max(cell.latest.snapshotSequence, list.snapshotSequence);
        cell.latest.serverTimestamp = std::max(cell.latest.serverTimestamp, list.serverTimestamp);
    ...
    void ActorSync::onActorCombatRequest(const ActorList& list)
    {
        ...
        for (const BaseActor& actorState : list.actors)
        {
            const ActorInstanceId victimActorNetId = actorNetIdForActorState(actorState);
            if (victimActorNetId == 0 || !hasAuthorityForActor(victimActorNetId, list.cellId))
            {
                Log(Debug::Verbose) << "[MP] ActorSync: CombatRequest ignored by non-owner"
            ...
            const auto& requestedAttackerPos = actorState.position.pos;
            const auto& requestedHitPos = actorState.attack.hitPos;
            const bool hasRequestedAttackGeometry
                = std::isfinite(requestedAttackerPos[0]) && std::isfinite(requestedAttackerPos[1])
                && std::isfinite(requestedAttackerPos[2]) && std::isfinite(requestedHitPos[0])
                && std::isfinite(requestedHitPos[1]) && std::isfinite(requestedHitPos[2])
                && (std::abs(requestedAttackerPos[0]) > 0.001f
                    || std::abs(requestedAttackerPos[1]) > 0.001f
                    || std::abs(requestedAttackerPos[2]) > 0.001f);
            const float requestedDx = requestedAttackerPos[0] - requestedHitPos[0];
            const float requestedDy = requestedAttackerPos[1] - requestedHitPos[1];
            const float requestedDz = requestedAttackerPos[2] - requestedHitPos[2];
            const float requestedAttackDistanceSq
                = requestedDx * requestedDx + requestedDy * requestedDy + requestedDz * requestedDz;
            const bool proxyMeleeRangeValid = attackerDistanceSq >= 0.f
                && attackerDistanceSq <= kMaxReplicatedMeleeHitDistance * kMaxReplicatedMeleeHitDistance;
            const bool requestMeleeRangeValid = hasRequestedAttackGeometry
                && requestedAttackDistanceSq
                    <= kMaxReplicatedMeleeHitDistance * kMaxReplicatedMeleeHitDistance;
            if (actorState.attack.damage > 0.f && isMeleeAttackType(actorState.attack.type)
                && !proxyMeleeRangeValid && !requestMeleeRangeValid)
            {
                Log(Debug::Warning) << "[MP] ActorSync: CombatRequest rejected outside melee range"
                                    ...
                                    << " distance=" << std::sqrt(attackerDistanceSq)
                                    << " requestDistance="
                                    << (hasRequestedAttackGeometry ? std::sqrt(requestedAttackDistanceSq) : -1.f)
                                    << " max=" << kMaxReplicatedMeleeHitDistance;
                continue;
            }
            ...
            Log(Debug::Info) << "[MP] ActorSync: CombatRequest from guid=" << list.authorityGuid
                             ...
                             << " mpNum=" << actorState.mpNum
                              ...
                              << " requestDistance="
                              << (hasRequestedAttackGeometry ? std::sqrt(requestedAttackDistanceSq) : -1.f)
                              << " usedRequestGeometry=" << (!proxyMeleeRangeValid && requestMeleeRangeValid);
    ...
    bool ActorSync::hasAuthorityForActor(ActorInstanceId actorNetId, const std::string& cellId) const
    {
        const auto leaseIt = mActorAuthorityGuids.find(actorNetId);
        if (leaseIt == mActorAuthorityGuids.end())
    ...
    bool ActorSync::hasAuthorityForMpNum(uint32_t mpNum, const std::string& cellId) const
    {
        if (mpNum == 0)
            return hasAuthority(cellId);
        return hasAuthorityForActor(
            packActorInstanceKey({ ActorKeyKind::SpawnedMpNum, mpNum }), cellId);
    }

    bool ActorSync::hasAuthorityForObject(const MWWorld::Ptr& ptr) const
    {
        if (ptr.isEmpty() || !ptr.getClass().isActor())
            return false;
        const std::string cellId = getActorAuthorityCellId(ptr);
        const ActorInstanceId actorNetId = actorNetIdForPtr(cellIdForPtr(ptr), ptr);
        return actorNetId != 0 && hasAuthorityForActor(actorNetId, cellId);
    }

    std::string ActorSync::getActorAuthorityCellId(const MWWorld::Ptr& ptr) const
    {
        if (ptr.isEmpty() || !ptr.getClass().isActor())
            return {};

        const std::string boundCellId = cellIdForPtr(ptr);
        const ActorInstanceId actorNetId = actorNetIdForPtr(boundCellId, ptr);
        if (actorNetId != 0)
        {
            const auto runtimeIt = mActorsByNetId.find(actorNetId);
            if (runtimeIt != mActorsByNetId.end() && !runtimeIt->second.state.cellId.empty())
                return runtimeIt->second.state.cellId;
        }
        ...
        const osg::Vec3f& hitPos, int attackType, float attackStrength)
    {
        ...
        if (hasAuthorityForObject(victim))
        {
            Log(Debug::Verbose) << "[MP] ActorSync: sendCombatRequest skipped: we have authority for victim"
                                << " refId=" << victim.getCellRef().getRefId().serializeText()
                                << " cell=" << cellId;
            return;
        }
        ...
        request.authorityGuid = mwmp::Main::get().getPlayerSync().localPlayer().guid;

        BaseActor requestedActor;
        requestedActor.refId = victim.getCellRef().getRefId().serializeText();
        requestedActor.refNum = victimMpNum != 0 ? 0 : victimRefNum;
        ...
        if (MWBase::World* world = MWBase::Environment::get().getWorld())
        {
            ...
            if (!localPlayer.isEmpty())
            {
                const auto& attackerPos = localPlayer.getRefData().getPosition();
                for (int i = 0; i < 3; ++i)
                {
                    requestedActor.position.pos[i] = attackerPos.pos[i];
                    requestedActor.position.rot[i] = attackerPos.rot[i];
                }
            }
        }
        ...
        Log(Debug::Info) << "[MP] ActorSync: sent CombatRequest for victim=" << requestedActor.refId
                         << " localRefNum=" << victim.getCellRef().getRefNum().mIndex
                         << " canonicalRefNum=" << requestedActor.refNum
        ...
        int attackType, const MWWorld::Ptr& npcAttacker)
    {
        ...
        {
            const ActorInstanceKey attackerKey
                = unpackActorInstanceId(actorNetIdForPtr(cellId, npcAttacker));
            if (attackerKey.kind == ActorKeyKind::VanillaRefNum)
                npcActor.refNum = attackerKey.id;
        }
        ...
        const std::string& castAnim, const MWWorld::Ptr& target, bool release)
    {
        ...
        castList.authorityGuid = mwmp::Main::get().getPlayerSync().localPlayer().guid;
        ...
        {
            castActor.mpNum = canonicalKey.id;
            castActor.refNum = 0;
        }
        else if (canonicalKey.kind == ActorKeyKind::VanillaRefNum)
            castActor.refNum = canonicalKey.id;
    ...
    void ActorSync::queueSnapshot(ActorRuntime& actor, const BaseActor& state, const ActorList& list)
    {
        ...
        snapshot.migrationGeneration = state.migrationGeneration;
        ...
        snapshot.currentAnimCompletion = state.animFlags.currentAnimCompletion;
    ...
    void ActorSync::mergeActorState(ActorRuntime& actor, const BaseActor& state, bool includeTransform)
    {
        ...
        if (includeTransform)
        {
            ...
            const bool holdingSyncedSpecialIdle = isIdleAnimGroup(actor.state.animFlags.currentAnimGroup)
                && !isBaseIdleAnimGroup(actor.state.animFlags.currentAnimGroup);
            const bool incomingReliableNonIdleGroup = isReliablePresentationAnimGroup(state.animFlags.currentAnimGroup)
                && !isIdleAnimGroup(state.animFlags.currentAnimGroup);
            ...
            if (holdingSyncedSpecialIdle && !incomingReliableNonIdleGroup
                && !state.isAttackingOrCasting
                && !state.hasWeaponDrawn
                && !state.hasSpellReadied
                && state.animFlags.movementFlags == 0)
            {
                actor.state.isMoving = false;
                actor.state.velocity = Velocity {};
                actor.state.animFlags.animFwd = 0.f;
                actor.state.animFlags.animSide = 0.f;
            }
            else
            {
                ...
                actor.state.animFlags.currentAnimCompletion = state.animFlags.currentAnimCompletion;
            }
        }
    }
    ...
    void ActorSync::advanceSmoothing(ActorRuntime& actor, float dt)
    {
        ...
        const auto applySnapshotPresentation = [&actor, dt](const BufferedSnapshot& snapshot)
        {
            ...
            const bool holdingSyncedSpecialIdle = isIdleAnimGroup(actor.state.animFlags.currentAnimGroup)
                && !isBaseIdleAnimGroup(actor.state.animFlags.currentAnimGroup);
            const bool snapshotHasReliableNonIdleGroup = isReliablePresentationAnimGroup(snapshot.currentAnimGroup)
            ...
            // PresentationV2 is the reliable owner of animation group/phase. Position snapshots
            // may remain buffered across a presentation change, so applying their cached
            // presentation fields afterward would roll the remote actor back to an older idle
            // until the next reliable presentation refresh. Reliable presentation also wins
            // timestamp ties because the two streams can be stamped in the same server tick.
            const bool doesNotPostdateReliablePresentation = snapshot.serverTimestamp != 0
                && actor.lastPresentationServerTimestamp != 0
                && snapshot.serverTimestamp <= actor.lastPresentationServerTimestamp;
            if (!doesNotPostdateReliablePresentation)
            {
                actor.state.animFlags.currentAnimGroup = snapshot.currentAnimGroup;
                actor.state.animFlags.currentAnimCompletion = snapshot.currentAnimCompletion;
            }
        };
    ...
    void ActorSync::fastForwardRuntimeToLatestSnapshot(ActorRuntime& actor, const char* reason, uint64_t eventTimestamp)
    {
        ...
        actor.state.animFlags.currentAnimCompletion = latest.currentAnimCompletion;
        actor.latestSnapshotAge = 0.f;
        actor.hasStationaryHoldPosition = false;
    ...
    uint32_t ActorSync::mappedMpNumForPtr(const std::string& cellId, const MWWorld::Ptr& ptr) const
    {
        ...
        {
            if (!mappedPtr.isEmpty() && (mappedPtr == ptr || sameLocalActorReference(mappedPtr, ptr)))
                return mpNum;
        }


        // A moved object can acquire a new Ptr wrapper before rememberServerSpawnedActor
        // installs the destination-cell key.  Search existing keys without the cell prefix,
        ...
        {
            if (localKey.size() < localKeySuffix.size()
                || localKey.compare(localKey.size() - localKeySuffix.size(), localKeySuffix.size(), localKeySuffix) != 0)
                continue;
            if (matchedMpNum != 0 && matchedMpNum != candidateMpNum)
                return 0;
            matchedMpNum = candidateMpNum;
        }
        if (matchedMpNum != 0)
            return matchedMpNum;

        // Cell changes can replace the local Ptr wrapper before the per-cell lookup tables are
        // rebuilt.  Keep the stable network identity by consulting the canonical runtime binding;
        // otherwise Lua falls back to cell authority during the crossing and transfers AI control
        // to the observer even though the server-side actor lease is still valid.
        for (const auto& [actorNetId, runtime] : mActorsByNetId)
        {
            if (runtime.state.mpNum == 0 || runtime.boundActor.isEmpty())
                continue;
            if (runtime.boundActor == ptr || sameLocalActorReference(runtime.boundActor, ptr))
                return runtime.state.mpNum;
        }
    ...
    void ActorSync::applyBootstrapDeathState(ActorRuntime& actor)
    {
        ...
        bool deathAppliedToBinding = false;
        if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
            baseNode->getUserValue(BootstrapDeathAppliedUserValue, deathAppliedToBinding);
        ...
        // visible fall/snap loop on non-authority clients.
        if (firstBootstrapApply && actor.hasAuthoritativeTransform)
        {
        ...
        MWRender::Animation* animation = world->getAnimation(actor.boundActor);

        // A baseline corpse may have no recorded death group (notably actors
        // that started dead in content). Pick the same deterministic fallback
        // on every client, then prefer the first available death animation if
        // that model does not provide the selected variant.
        std::string corpseDeathGroup = actor.state.deathAnimGroup.empty()
            ? fallbackDeathAnimGroup(actor.state)
            : actor.state.deathAnimGroup;
        if (animation && !animation->hasAnimation(corpseDeathGroup))
        {
            const uint32_t seed = actor.state.mpNum != 0 ? actor.state.mpNum : actor.state.refNum;
            for (uint32_t offset = 0; offset < 5; ++offset)
            {
                const std::string candidate = "death" + std::to_string(((seed + offset) % 5) + 1);
                if (animation->hasAnimation(candidate))
                {
                    corpseDeathGroup = candidate;
                    break;
                }
            }
        }

        if (!corpseDeathGroup.empty())
        {
            actor.state.deathAnimGroup = corpseDeathGroup;
            actor.appliedDeathAnimGroup = corpseDeathGroup;
            if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
        ...
        if (animation)
        {
            ...
            // Setting health to zero is observed by CharacterController on its
            // next update. Without sampling the final death pose here, a newly
            // inserted corpse renders upright for one frame and then visibly
            // drops when that update runs. Baseline corpses are already dead,
            // so skip directly to the stop key and update the skeleton now.
            if (!actor.appliedDeathAnimGroup.empty()
                && animation->hasAnimation(actor.appliedDeathAnimGroup))
            {
                animation->play(actor.appliedDeathAnimGroup,
                    ...
                    1.f,
                    0u);
                animation->runAnimation(0.f);

                Log(Debug::Verbose) << "[MP] ActorSync: sampled bootstrap corpse final pose"
                                 << " refId=" << actor.state.refId
                                 << " refNum=" << actor.state.refNum
                                 << " mpNum=" << actor.state.mpNum
                                 << " deathAnim='" << actor.appliedDeathAnimGroup << "'";
    ...
    void ActorSync::sendAuthoritativeActorUpdates(const std::string& cellId, CellRuntime& cell, float dt)
    {
        ...
        else if (cell.outboundCellId != cellId)
        {
            ...
            cell.priorityPositionSendCursor = 0;
            cell.actors.clear();
            cell.lastSentActorListKeys.clear();
        ...
        for (auto& [actorKey, actor] : cell.actors)
        {
            ...
            if (actor.state.mpNum != 0)
            {
                if (resolveActorBinding(cell.outboundCellId, actor) && !actor.boundActor.isEmpty())
                {
                    // Actors first received through ActorCellChange can exist only
                    // in this cell runtime until a later identity packet promotes
                    // them into mActorsByNetId. They still need full authority
                    // simulation immediately; otherwise the controller is culled
                    // and the handoff Travel package never produces movement.
                    if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
                    {
                        bool wasAuthoritativeActor = false;
                        const bool hadAuthorityFlag
                        ...
                        if (!hadAuthorityFlag || !wasAuthoritativeActor)
                        {
                            Log(Debug::Verbose) << "[MP] ActorSync: enabled cell-runtime authoritative actor simulation"
                    ...
                    restoreAuthorityActorAiIfNeeded(cell.outboundCellId, actor.boundActor,
                        actor.state, actor.savedPuppetAiSequence.get());
                }
            }
        }

        // Generated leveled-creature children receive local runtime refNums in
        // spawn order. Chance None and RNG differences make those numbers refer
        // to unrelated spawn points across clients. Key each child by the stable
        // placed leveled-list reference that owns it instead.
        std::unordered_map<uint32_t, uint32_t> leveledSpawnerRefNumByChildRefNum;
        targetCell->forEach([&](MWWorld::Ptr ptr) -> bool
        {
            if (ptr.isEmpty() || ptr.getType() != ESM::CreatureLevList::sRecordId)
                return true;
            ...
            const bool canonicalChanceNone = chanceNoneCellIt != mChanceNoneLeveledSpawnersByCell.end()
                && chanceNoneCellIt->second.count(spawnerActorNetId) != 0;
            if (canonicalChanceNone)
            {
                if (!spawnedActor.isEmpty())
                {
                    const std::string localRefId
                        = spawnedActor.getCellRef().getRefId().serializeText();
                    world->deleteObject(spawnedActor);
                    leveledClass.setSpawnedActor(ptr, MWWorld::Ptr());
                    mReconciledVanillaActorsByNetId.erase(spawnerActorNetId);
                    Log(Debug::Info) << "[MP] ActorSync: suppressed re-rolled Chance None child"
                                     << " cell=" << cell.outboundCellId
                                     << " spawnerRefNum=" << spawnerRefNum
                                     << " localRefId=" << localRefId
                                     << " actorNetId=" << spawnerActorNetId;
                }
                return true;
            }
            if (!spawnedActor.isEmpty() && spawnedActor.getClass().isActor())
            {
                leveledSpawnerRefNumByChildRefNum[spawnedActor.getCellRef().getRefNum().mIndex]
                    = spawnerRefNum;
            }
            return true;
        });

        // A replaced leveled-list child remains in the CellStore with a zero
        // count after World::deleteObject(). Never let that stale object reuse
        // the stable spawner identity alongside the live replacement.
        ...
        auto collectAuthorityActor = [&](MWWorld::Ptr ptr) -> bool
        {
            ...
            // Observer-side leveled-list reconciliation creates a normal local
            // actor with a new runtime refNum. If this client later becomes the
            // authority, retain the placed reference's authoritative identity
            // instead of advertising the replacement as a brand-new actor.
            for (const auto& [reconciledNetId, reconciledPtr] : mReconciledVanillaActorsByNetId)
            {
                if (reconciledPtr.isEmpty()
                    || (!(reconciledPtr == ptr) && !sameLocalActorObject(reconciledPtr, ptr)))
                    continue;

                const auto runtimeIt = mActorsByNetId.find(reconciledNetId);
                if (runtimeIt != mActorsByNetId.end() && runtimeIt->second.state.mpNum == 0
                    && runtimeIt->second.state.refNum != 0)
                {
                    actor.refId = runtimeIt->second.state.refId;
                    actor.refNum = runtimeIt->second.state.refNum;
                }
                break;
            }
            ...
            const ActorInstanceId canonicalActorNetId = actorNetIdForActorState(actor);
            if (mappedMpNum == 0
                && !hasAuthorityForActor(canonicalActorNetId, cell.outboundCellId))
                return true;
            if (mappedMpNum != 0)
            {
                MWWorld::Ptr canonicalPtr;
                const ActorInstanceId actorNetId = packActorInstanceKey({ ActorKeyKind::SpawnedMpNum, mappedMpNum });
                if (!hasAuthorityForActor(actorNetId, cell.outboundCellId))
                    return true;
                auto primaryIt = mActorsByNetId.find(actorNetId);
                ...
                if (primaryIt != mActorsByNetId.end()
                    && !primaryIt->second.state.cellId.empty()
                    && primaryIt->second.state.cellId != cell.outboundCellId)
                {
                    // The server has committed this actor to another cell. The
                    // old Ptr can remain visible until its cell unloads, but it
                    // must not be rediscovered as a source-cell authority actor.
                    if (auto* baseNode = ptr.getRefData().getBaseNode())
                    {
                        baseNode->setUserValue("mp_authoritative_actor", false);
            ...
            const ESM::AiSequence::AiSequence* savedPuppetAiSequence
                = previousRuntime != cell.actors.end() && previousRuntime->second.savedPuppetAiSequence
                    && !previousRuntime->second.savedPuppetAiSequence->mPackages.empty()
                ? previousRuntime->second.savedPuppetAiSequence.get()
                : nullptr;

            if (previousRuntime != cell.actors.end()
                && previousRuntime->second.authoritySyncedIdleHandoffTimer > 0.f)
            {
                previousRuntime->second.authoritySyncedIdleHandoffTimer
                    = std::max(0.f, previousRuntime->second.authoritySyncedIdleHandoffTimer - std::max(0.f, dt));
                if (previousRuntime->second.authoritySyncedIdleHandoffTimer <= 0.f)
                    clearSyncedIdleAuthorityHandoffNode(cell.outboundCellId, ptr, actor, "handoff-grace-expired");
            }

            const bool preservedSyncedIdleHandoff
                = restoreAuthorityActorAiIfNeeded(cell.outboundCellId, ptr, actor, savedPuppetAiSequence);
            if (preservedSyncedIdleHandoff && previousRuntime != cell.actors.end())
            {
                static constexpr float kAuthoritySyncedIdleHandoffGrace = 2.0f;
                previousRuntime->second.authoritySyncedIdleHandoffTimer
                    = std::max(previousRuntime->second.authoritySyncedIdleHandoffTimer,
                        kAuthoritySyncedIdleHandoffGrace);
            }
            ...
            actor.ai.targetId.clear();
            actor.ai.targetMpNum = 0;
            actor.ai.duration = 0.f;
            actor.ai.reset = false;
            MWMechanics::AiSequence& aiSequence = stats.getAiSequence();
            MWWorld::Ptr aiTarget;
            if (!actor.isDead)
            {
                switch (aiSequence.getTypeId())
                {
                    case MWMechanics::AiPackageTypeId::Follow:
                        actor.ai.type = BaseActor::AIAction::Type::Follow;
                        break;
                    ...
                    case MWMechanics::AiPackageTypeId::Wander:
                        actor.ai.type = BaseActor::AIAction::Type::Wander;
                        break;
                    default:
                        break;
                }

                if (actor.ai.type == BaseActor::AIAction::Type::Combat)
                    aiSequence.getCombatTarget(aiTarget);
                else if ((actor.ai.type == BaseActor::AIAction::Type::Follow
                             || actor.ai.type == BaseActor::AIAction::Type::Escort
                             || actor.ai.type == BaseActor::AIAction::Type::Pursue)
                    && !aiSequence.isEmpty())
                {
                    aiTarget = aiSequence.getActivePackage().getTarget();
                }

                if (actor.ai.type == BaseActor::AIAction::Type::None && aiSequence.isInCombat())
                {
                    actor.ai.type = BaseActor::AIAction::Type::Combat;
                    aiSequence.getCombatTarget(aiTarget);
                }
                ...
                if (!aiTarget.isEmpty())
                {
                    if (aiTarget == playerPtr)
                    {
                        const uint32_t localGuid = mwmp::Main::get().getPlayerSync().localPlayer().guid;
                        actor.ai.targetId = std::string("mp_remote_") + std::to_string(localGuid);
                    }
                    else
                        actor.ai.targetId = aiTarget.getCellRef().getRefId().serializeText();
            ...
            if (!actor.isDead)
            {
                if (auto* baseNode = ptr.getRefData().getBaseNode())
                {
                    ...
                    std::string attackGroup;
                    if (baseNode->getUserValue("mp_attack_pending", attackPending) && attackPending)
                    {
                        baseNode->setUserValue("mp_attack_pending", false);
                        baseNode->getUserValue("mp_attack_type", attackType);
                        baseNode->getUserValue("mp_attack_group", attackGroup);
                        actor.attack.pressed = true;
                        actor.attack.attackAnimation
                            = !attackGroup.empty() ? attackGroup : attackType;
                    }
                }
            }
            ...
            const bool sampledSpecialIdle = isIdleAnimGroup(actor.animFlags.currentAnimGroup)
                && !isBaseIdleAnimGroup(actor.animFlags.currentAnimGroup);
            if (sampledSpecialIdle)
            {
                if (const auto* baseNode = ptr.getRefData().getBaseNode())
                {
                    std::string controllerIdle;
                    if (baseNode->getUserValue("mp_current_idle_group", controllerIdle)
                        && controllerIdle == actor.animFlags.currentAnimGroup)
                    {
                        baseNode->getUserValue("mp_current_idle_single_cycle", actor.animFlags.idleSingleCycle);
                        baseNode->getUserValue("mp_current_idle_event_parity", actor.animFlags.idleEventParity);
                    }
                }
            }
            ...
            const bool sampledLocomotion = isLocomotionAnimGroup(rawLowerBodyGroup)
                && !isBaseIdleAnimGroup(rawLowerBodyGroup);
            bool controllerLocomotion = false;
            bool haveControllerLocomotion = false;
            if (!actor.isDead)
            {
                if (const auto* baseNode = ptr.getRefData().getBaseNode())
                {
                    haveControllerLocomotion = baseNode->getUserValue("mp_current_locomotion", controllerLocomotion);
                    if (haveControllerLocomotion)
                    {
                        actor.isMoving = sampledLocomotion && controllerLocomotion;
                        actor.animFlags.animFwd = 0.f;
                        actor.animFlags.animSide = 0.f;
                        if (actor.isMoving)
                        {
                            baseNode->getUserValue("mp_current_locomotion_fwd", actor.animFlags.animFwd);
                            baseNode->getUserValue("mp_current_locomotion_side", actor.animFlags.animSide);
                        }
                    }
                }
            }
            const bool confirmedLocomotion = sampledLocomotion && actor.isMoving;
            const bool stationaryIdleContext = !actor.isDead && !actor.isAttackingOrCasting
                && !actor.hasWeaponDrawn && !actor.hasSpellReadied
                && actor.animFlags.movementFlags == 0;
            const bool specialIdleLocomotionBlock = sampledSpecialIdle
                || (stationaryIdleContext && !sampledLocomotion
                    && (previousSpecialIdle || rawLowerBodyGroup == "idle"));
            ...
            if (previousRuntime != cell.actors.end())
            {
                ...
                static constexpr float kAuthorityReliableAnimGroupLostGrace = 0.12f;
                ...
                const bool currentReliableGroup = isReliablePresentationAnimGroup(actor.animFlags.currentAnimGroup);
                const bool previousReliableGroup = isReliablePresentationAnimGroup(previous.state.animFlags.currentAnimGroup);
                if (currentReliableGroup)
                    previous.authorityReliableAnimGroupLostTimer = 0.f;
                else if (!actor.isDead && previousReliableGroup
                    && !(previousSpecialIdle && confirmedLocomotion)
                    && previous.authorityReliableAnimGroupLostTimer < kAuthorityReliableAnimGroupLostGrace)
                {
                    previous.authorityReliableAnimGroupLostTimer += std::max(0.f, dt);
                    const bool holdingReliableGroup =
                        previous.authorityReliableAnimGroupLostTimer < kAuthorityReliableAnimGroupLostGrace;

                    if (isWatchedPresentationActor(actor, 0))
                    {
                        Log(Debug::Info) << "[MPWATCH] ActorSync v2: authority lost-group grace"
                                         << " refId=" << actor.refId
                                         << " rawLowerBody='" << rawLowerBodyGroup << "'"
                                         << " sampledGroup='" << actor.animFlags.currentAnimGroup << "'"
                                         << " previousGroup='" << previous.state.animFlags.currentAnimGroup << "'"
                                         << " timerMs=" << (previous.authorityReliableAnimGroupLostTimer * 1000.f)
                                         << " graceMs=" << (kAuthorityReliableAnimGroupLostGrace * 1000.f)
                                         << " holding=" << holdingReliableGroup;
                    }

                    if (holdingReliableGroup)
                    {
                        actor.animFlags.currentAnimGroup = previous.state.animFlags.currentAnimGroup;
                        actor.animFlags.currentAnimCompletion = previous.state.animFlags.currentAnimCompletion;
                        actor.animFlags.idleSingleCycle = previous.state.animFlags.idleSingleCycle;
                        actor.animFlags.idleEventParity = previous.state.animFlags.idleEventParity;
                    }
                }
                else
                    previous.authorityReliableAnimGroupLostTimer = kAuthorityReliableAnimGroupLostGrace;

                // Expiry means the old sample is no longer usable, not that a
                // stationary actor has surrendered its presentation. Bound an
                // unknown gap with base idle instead of looping the old special
                // idle forever. Real movement and action interruptions bypass it.
                if ((actor.animFlags.currentAnimGroup.empty()
                        || isLocomotionAnimGroup(actor.animFlags.currentAnimGroup)) && !actor.isMoving
                    && stationaryIdleContext
                    && (haveControllerLocomotion || previousSpecialIdle
                        || previous.state.animFlags.currentAnimGroup == "idle"))
                {
                ...
                if (isWatchedPresentationActor(actor, 0)
                    && previous.state.animFlags.currentAnimGroup != actor.animFlags.currentAnimGroup
                    && (previousSpecialIdle || sampledSpecialIdle))
                {
                    std::string controllerIdleGroup;
                    if (const auto* baseNode = ptr.getRefData().getBaseNode())
                        baseNode->getUserValue("mp_current_idle_group", controllerIdleGroup);
                    float previousCompletion = -1.f;
                    bool previousPresent = false;
                    bool previousPlaying = false;
                    if (MWRender::Animation* animObj = world->getAnimation(ptr))
                    {
                        previousPresent = animObj->getInfo(previous.state.animFlags.currentAnimGroup, &previousCompletion);
                        previousPlaying = animObj->isPlaying(previous.state.animFlags.currentAnimGroup);
                    }
                    Log(Debug::Info) << "[MPWATCH] ActorSync v2: authority idle transition"
                                     << " refId=" << actor.refId
                                     << " previousGroup='" << previous.state.animFlags.currentAnimGroup << "'"
                                     << " rawLowerBody='" << rawLowerBodyGroup << "'"
                                     << " controllerIdle='" << controllerIdleGroup << "'"
                                     << " group='" << actor.animFlags.currentAnimGroup << "'"
                                     << " previousPresent=" << previousPresent
                                     << " previousPlaying=" << previousPlaying
                                     << " previousCompletion=" << (previousPresent ? previousCompletion : -1.f)
                                     << " lastSampleCompletion=" << previous.state.animFlags.currentAnimCompletion
                                     << " moving=" << actor.isMoving
                                     << " controllerLocomotion=" << controllerLocomotion
                                     << " reason=" << (sampledSpecialIdle ? "replacement"
                                         : confirmedLocomotion ? "locomotion"
                                         : !stationaryIdleContext ? "interruption"
                                         : rawLowerBodyGroup == "idle" ? "base-idle" : "sampling-gap");
                }
            }
            ...
            if (actor.mpNum != 0)
            {
                ...
                auto primaryIt = mActorsByNetId.find(actorNetId);
                if (primaryIt != mActorsByNetId.end())
                    actor.migrationGeneration = primaryIt->second.migrationGeneration;
            ...
            const ActorInstanceId outgoingActorNetId = actorNetIdForActorState(actor);
            if (const auto runtimeIt = mActorsByNetId.find(outgoingActorNetId);
                runtimeIt != mActorsByNetId.end())
            {
                actor.migrationGeneration = runtimeIt->second.migrationGeneration;
            }
            else if (previousRuntime != cell.actors.end())
            {
                actor.migrationGeneration = previousRuntime->second.migrationGeneration;
            }

            if (outgoingActorNetId == 0
                || !collectedAuthorityActorNetIds.insert(outgoingActorNetId).second)
            {
                if (outgoingActorNetId != 0)
                {
                    Log(Debug::Warning) << "[MP] ActorSync: skipped duplicate authority actor identity"
                                        << " cell=" << cell.outboundCellId
                                        << " actorNetId=" << outgoingActorNetId
                                        << " refId=" << actor.refId
                                        << " refNum=" << actor.refNum
                                        << " localRefNum=" << ptr.getCellRef().getRefNum().mIndex;
                }
                return true;
            }
        ...
        targetCell->forEach(collectAuthorityActor);

        // A walking actor can cross an exterior boundary while its Ptr remains bound
        // to the original CellStore. Derive the destination from world coordinates;
        // waiting for Ptr::getCell() to change prevents the canonical handoff.
        for (auto& actorEntry : mActorsByNetId)
        {
            if (!hasAuthorityForActor(actorEntry.first, cell.outboundCellId))
                continue;
            ActorRuntime& runtime = actorEntry.second;
            if (runtime.state.mpNum == 0
                || runtime.state.cellId != cell.outboundCellId
                || runtime.boundActor.isEmpty()
                || !runtime.boundActor.getRefData().isEnabled())
            {
                continue;
            }

            // The reliable cell commit can advance the canonical v2 runtime
            // before World::moveObject transfers its Ptr out of the previous
            // CellStore. targetCell->forEach() cannot see that actor, even
            // though authority AI is already moving it in this cell. Always
            // collect the canonical primary runtime so observers keep
            // receiving position snapshots throughout repeated handoffs.
            collectAuthorityActor(runtime.boundActor);

            const auto& position = runtime.boundActor.getRefData().getPosition();
            Position boundPosition;
            for (int i = 0; i < 3; ++i)
                boundPosition.pos[i] = position.pos[i];
            const std::string positionCellId = exteriorCellIdForPosition(boundPosition);
            constexpr float kCellChangeBorderHysteresis = 64.f;
            const float borderDistance = exteriorCellBorderDistance(boundPosition);
            if (positionCellId == cell.outboundCellId)
            {
                if (borderDistance > kCellChangeBorderHysteresis)
                {
                    runtime.previousCellChangeCellId.clear();
                    runtime.cellChangeReverseGuardTimer = 0.f;
                    runtime.cellChangeReverseGuardLogged = false;
            ...
            if (positionCellId.empty()
                || borderDistance <= kCellChangeBorderHysteresis
                || !isExteriorActorCellId(cell.outboundCellId)
                || !isExteriorActorCellId(positionCellId))
                continue;

            if (positionCellId == runtime.previousCellChangeCellId
                && runtime.cellChangeReverseGuardTimer > 0.f)
            {
                if (!runtime.cellChangeReverseGuardLogged)
                {
                    runtime.cellChangeReverseGuardLogged = true;
                    Log(Debug::Info) << "[MP] ActorSync: suppressed provisional reverse ActorCellChange"
                                     << " refId=" << runtime.state.refId
                                     << " mpNum=" << runtime.state.mpNum
                                     << " canonical=" << cell.outboundCellId
                                     << " previous=" << positionCellId
                                     << " borderDistance=" << borderDistance;
                }
                continue;
            }

            if (runtime.pendingCellChangeDestination != positionCellId
                || runtime.pendingCellChangeRetryTimer <= 0.f)
            {
                BaseActor movedActor = runtime.state;
                // collectAuthorityActor() above captured this frame's live
                // velocity, locomotion axes, presentation and AI state. Use
                // that sample for the reliable migration commit instead of
                // the primary runtime's previous network state. Otherwise
                // observers rebase to a stationary border sample until the
                // destination authority's first position packet arrives.
                const auto sentPrimaryIt = sentServerSpawnedActors.find(runtime.state.mpNum);
                if (sentPrimaryIt != sentServerSpawnedActors.end()
                    && sentPrimaryIt->second.outgoingIndex < outgoing.actors.size())
                {
                    movedActor = outgoing.actors[sentPrimaryIt->second.outgoingIndex];
                }
                // Crossing the cell boundary can briefly clear the engine's
                // movement settings even though the actor still has forward
                // velocity. Preserve the last authored locomotion sample in the
                // reliable commit so the destination authority and observers do
                // not render an idle frame at the handoff.
                const bool movedActorHasAxes = std::abs(movedActor.animFlags.animFwd) > 0.1f
                    || std::abs(movedActor.animFlags.animSide) > 0.1f;
                const bool runtimeHasAxes = std::abs(runtime.state.animFlags.animFwd) > 0.1f
                    || std::abs(runtime.state.animFlags.animSide) > 0.1f;
                if (movedActor.isMoving && !movedActorHasAxes
                    && runtime.state.isMoving && runtimeHasAxes)
                {
                    movedActor.animFlags.animFwd = runtime.state.animFlags.animFwd;
                    movedActor.animFlags.animSide = runtime.state.animFlags.animSide;
                    movedActor.animFlags.movementFlags |= runtime.state.animFlags.movementFlags
                        & (AnimFlags::MF_RUN | AnimFlags::MF_SNEAK);
                    if ((movedActor.animFlags.currentAnimGroup.empty()
                            || isIdleAnimGroup(movedActor.animFlags.currentAnimGroup))
                        && isLocomotionAnimGroup(runtime.state.animFlags.currentAnimGroup))
                    {
                        movedActor.animFlags.currentAnimGroup = runtime.state.animFlags.currentAnimGroup;
                        movedActor.animFlags.currentAnimCompletion
                            = runtime.state.animFlags.currentAnimCompletion;
                    }
                }
                else if (movedActor.isMoving && !movedActorHasAxes)
                {
                    // AI path updates can clear mPosition for the exact frame
                    // that crosses a cell border. TES3MP carries DedicatedActor
                    // direction into the new LocalActor; reconstruct the same
                    // body-relative direction from authoritative world velocity.
                    const float velocityX = movedActor.velocity.linear[0];
                    const float velocityY = movedActor.velocity.linear[1];
                    const float planarSpeed = std::sqrt(
                        velocityX * velocityX + velocityY * velocityY);
                    if (planarSpeed > 1.f)
                    {
                        const float yaw = movedActor.position.rot[2];
                        const float forwardX = std::sin(-yaw);
                        const float forwardY = std::cos(-yaw);
                        const float sideX = std::cos(-yaw);
                        const float sideY = -std::sin(-yaw);
                        movedActor.animFlags.animFwd = std::clamp(
                            (velocityX * forwardX + velocityY * forwardY) / planarSpeed,
                            -1.f, 1.f);
                        movedActor.animFlags.animSide = std::clamp(
                            (velocityX * sideX + velocityY * sideY) / planarSpeed,
                            -1.f, 1.f);
                    }
                    else
                    {
                        // A moving AI actor with no usable instantaneous velocity
                        // is overwhelmingly in a forward path-follow segment.
                        // Keeping one forward input frame is preferable to an
                        // unconditional stop at every authority conversion.
                        movedActor.animFlags.animFwd = 1.f;
                        movedActor.animFlags.animSide = 0.f;
                    }
                }
                movedActor.cellId = positionCellId;
                for (int i = 0; i < 3; ++i)
                {
                    movedActor.position.pos[i] = position.pos[i];
                    movedActor.position.rot[i] = position.rot[i];
                }

                if (!runtime.boundActor.isEmpty())
                {
                    MWMechanics::CreatureStats& moveStats = runtime.boundActor.getClass().getCreatureStats(runtime.boundActor);
                    MWMechanics::AiSequence& moveAiSequence = moveStats.getAiSequence();
                    MWWorld::Ptr combatTarget;
                    if (!moveStats.isDead() && moveAiSequence.getCombatTarget(combatTarget))
                    {
                        movedActor.ai.type = BaseActor::AIAction::Type::Combat;
                        if (!combatTarget.isEmpty())
                        {
                            MWBase::World* moveWorld = MWBase::Environment::get().getWorld();
                            if (moveWorld && combatTarget == moveWorld->getPlayerPtr())
                            {
                                const uint32_t localGuid = mwmp::Main::get().getPlayerSync().localPlayer().guid;
                                movedActor.ai.targetId = std::string("mp_remote_") + std::to_string(localGuid);
                            }
                            else
                                movedActor.ai.targetId = combatTarget.getCellRef().getRefId().serializeText();

                            const std::string targetCellId = cellIdForPtr(combatTarget);
                            movedActor.ai.targetMpNum = mappedMpNumForPtr(targetCellId, combatTarget);
                        }
                    }
                    else if (!moveStats.isDead() && moveAiSequence.isInCombat())
                        movedActor.ai.type = BaseActor::AIAction::Type::Combat;
                }

                bool movedActorLiveCombat = false;
                if (!runtime.boundActor.isEmpty())
                {
                    MWMechanics::CreatureStats& liveMoveStats = runtime.boundActor.getClass().getCreatureStats(runtime.boundActor);
                    MWMechanics::AiSequence& liveMoveAiSequence = liveMoveStats.getAiSequence();
                    movedActorLiveCombat = !liveMoveStats.isDead() && liveMoveAiSequence.isInCombat();
                    if (movedActorLiveCombat)
                        movedActor.ai.type = BaseActor::AIAction::Type::Combat;
                }

                const bool movedActorHasAiPackage = movedActor.mpNum != 0 || movedActor.isMoving || movedActor.ai.type != BaseActor::AIAction::Type::None;
                const bool movedActorHasCommitAxes = std::abs(movedActor.animFlags.animFwd) > 0.1f
                    || std::abs(movedActor.animFlags.animSide) > 0.1f;
                if (!movedActor.isDead && movedActorHasAiPackage && !movedActorHasCommitAxes)
                {
                    movedActor.isMoving = true;
                    movedActor.animFlags.animFwd = 1.f;
                    movedActor.animFlags.animSide = 0.f;
                }

                const bool sentCellChangeIsMoving = movedActor.isMoving;
                const float sentCellChangeAnimFwd = movedActor.animFlags.animFwd;
                const float sentCellChangeAnimSide = movedActor.animFlags.animSide;

                ActorList cellChange;
                cellChange.cellId = cell.outboundCellId;
                cellChange.isAuthority = true;
                cellChange.authorityGuid = outgoing.authorityGuid;
                cellChange.actors.push_back(std::move(movedActor));

                PacketActorCellChange packet;
                packet.setActorList(&cellChange);
                mClient.sendReliable(packet.encode());

                runtime.pendingCellChangeDestination = positionCellId;
                runtime.pendingCellChangeRetryTimer = 1.f;
                Log(Debug::Info) << "[MP] ActorSync: sent provisional ActorCellChange"
                                 << " refId=" << runtime.state.refId
                                 << " mpNum=" << runtime.state.mpNum
                                 << " from=" << cell.outboundCellId
                                 << " to=" << positionCellId
                                 << " pos=(" << position.pos[0] << "," << position.pos[1]
                                 << "," << position.pos[2] << ")"
                                 << " isMoving=" << sentCellChangeIsMoving
                                 << " axes=(" << sentCellChangeAnimSide << ","
                                 << sentCellChangeAnimFwd << ")";
            }
        ...
        // ActorList is a complete cell registry replacement on the server. A
        // per-actor lease owner must never send its partial owned subset, and
        // the cell authority must preserve actors currently simulated by a
        // different lease owner. Continuous position/presentation packets below
        // still use `outgoing`, which contains only actors owned by this client.
        if (hasAuthority(cell.outboundCellId))
        {
            ActorList completeList = outgoing;
            std::unordered_set<std::string> completeKeys;
            completeKeys.reserve(outgoing.actors.size() + 8);
            for (const BaseActor& actorState : outgoing.actors)
                completeKeys.insert(makeActorKey(actorState));

            for (const auto& [actorNetId, runtime] : mActorsByNetId)
            {
                if (runtime.state.cellId != cell.outboundCellId
                    || hasAuthorityForActor(actorNetId, cell.outboundCellId)
                    || hasMissingActorInstanceIdentity(runtime.state))
                    continue;

                const std::string actorKey = makeActorKey(runtime.state);
                if (completeKeys.insert(actorKey).second)
                    completeList.actors.push_back(runtime.state);
            }

            const bool actorKeySetChanged = !cell.initialListSent
                || completeKeys != cell.lastSentActorListKeys;
            if (actorKeySetChanged)
            {
                std::sort(completeList.actors.begin(), completeList.actors.end(),
                    [](const BaseActor& lhs, const BaseActor& rhs) {
                        return makeActorKey(lhs) < makeActorKey(rhs);
                    });
                PacketActorList listPacket;
                listPacket.setActorList(&completeList);
                mClient.sendReliable(listPacket.encode());
                const bool wasInitialList = !cell.initialListSent;
                cell.initialListSent = true;
                cell.lastSentActorListKeys = std::move(completeKeys);
                Log(Debug::Verbose) << "[MP] ActorSync: sent complete ActorList for "
                                    << cell.outboundCellId
                                    << " actors=" << completeList.actors.size()
                                    << " locallyOwned=" << outgoing.actors.size()
        ...
        ActorList aiUpdate;
        aiUpdate.cellId = outgoing.cellId;
        aiUpdate.isAuthority = true;
        aiUpdate.authorityGuid = outgoing.authorityGuid;
        ...
        for (const auto& actor : outgoing.actors)
        {
            ...
            {
                const BaseActor::AIAction& previousAi = prevIt->second.state.ai;
                const bool aiChanged = previousAi.type != actor.ai.type
                    || previousAi.targetId != actor.ai.targetId
                    || previousAi.targetMpNum != actor.ai.targetMpNum
                    || previousAi.reset != actor.ai.reset;
                if (aiChanged)
                {
                    aiUpdate.actors.push_back(actor);
                    Log(Debug::Info) << "[MP] ActorSync: queued ActorAI target update"
                                     << " actorNetId=" << actorNetIdForActorState(actor)
                                     << " refId=" << actor.refId
                                     << " mpNum=" << actor.mpNum
                                     << " cell=" << outgoing.cellId
                                     << " previousType=" << static_cast<int>(previousAi.type)
                                     << " type=" << static_cast<int>(actor.ai.type)
                                     << " previousTarget=" << previousAi.targetId
                                     << " target=" << actor.ai.targetId;
                }
            }
            ...
            const bool controllerAttackPending = actor.attack.pressed;
            const bool fallbackAttackEdge = !controllerAttackPending
                && actor.isAttackingOrCasting && !prevAttacking;
            if (controllerAttackPending || fallbackAttackEdge)
            {
                ...
                if (atkActor.attack.attackAnimation.empty())
                    atkActor.attack.attackAnimation = "slash";
                ...
                Log(Debug::Info) << "[MP] ActorSync: attack animation edge for " << actor.refId
                                 << " aiType=" << static_cast<int>(actor.ai.type)
                                 << " attackAnim=" << atkActor.attack.attackAnimation
                                 << " controllerHook=" << controllerAttackPending;
            ...
            if (actorNetId != 0)
            {
                ...
                {
                    if (MWRender::Animation* animObj = (prevIt != cell.actors.end() && !prevIt->second.boundActor.isEmpty())
                            ? world->getAnimation(prevIt->second.boundActor)
                            : nullptr)
                    {
                        float completion = -1.f;
                        if (animObj->getInfo(snapshot.currentAnimGroup, &completion))
                            snapshot.currentAnimCompletion = std::clamp(completion, 0.f, 1.f);
                    }
                }
                ...
                // Preserve an explicit stationary rest state. Empty is a clear,
                // and lets observers infer walking from residual root motion.
                const bool explicitBaseIdle = snapshot.currentAnimGroup == "idle"
                    && !snapshot.isMoving && !snapshot.isDead;
                if (!isReliablePresentationAnimGroup(reliableSnapshot.currentAnimGroup) && !explicitBaseIdle)
                    reliableSnapshot.currentAnimGroup.clear();
                if (!isIdleAnimGroup(reliableSnapshot.currentAnimGroup)
                    || isBaseIdleAnimGroup(reliableSnapshot.currentAnimGroup))
                    reliableSnapshot.presentationFlags &= ~ActorPresentationIdleMask;
                ...
                // Special idle variants can expose one-frame alternate groups at
                // loop/key boundaries. Hold those short candidates, but allow a
                // stable new group to become a real reliable presentation event.
                const bool previousSentSpecialIdle = !previousSentAnimGroup.empty()
                    && isIdleAnimGroup(previousSentAnimGroup)
                    && isReliablePresentationAnimGroup(previousSentAnimGroup);
                const bool currentSpecialIdle = isIdleAnimGroup(reliableSnapshot.currentAnimGroup)
                    && isReliablePresentationAnimGroup(reliableSnapshot.currentAnimGroup);
                const bool stableSpecialIdleContext = !reliableSnapshot.isAttackingOrCasting
                    && !reliableSnapshot.hasWeaponDrawn
                    && !reliableSnapshot.hasSpellReadied
                    && !reliableSnapshot.isDead
                    && reliableSnapshot.movementFlags == 0;
                const bool debounceSpecialIdleChange = !sampleRequested
                    && prevIt != cell.actors.end()
                    && previousSentSpecialIdle
                    && currentSpecialIdle
                    && reliableSnapshot.currentAnimGroup != previousSentAnimGroup
                    && stableSpecialIdleContext;
                if (debounceSpecialIdleChange)
                {
                    static constexpr float kSpecialIdleGroupChangeStableTime = 0.18f;
                    ActorRuntime& previous = prevIt->second;
                    if (previous.authorityPendingReliableAnimGroup != reliableSnapshot.currentAnimGroup)
                    {
                        previous.authorityPendingReliableAnimGroup = reliableSnapshot.currentAnimGroup;
                        previous.authorityPendingReliableAnimGroupTimer = 0.f;
                    }
                    previous.authorityPendingReliableAnimGroupTimer += std::max(0.f, dt);

                    if (previous.authorityPendingReliableAnimGroupTimer < kSpecialIdleGroupChangeStableTime)
                    {
                        reliableSnapshot.currentAnimGroup = previousSentAnimGroup;
                        reliableSnapshot.currentAnimCompletion = previous.state.animFlags.currentAnimCompletion;
                        reliableSnapshot.presentationFlags = (reliableSnapshot.presentationFlags & ~ActorPresentationIdleMask)
                            | (previous.lastSentPresentationFlags & ActorPresentationIdleMask);
                    }
                    else
                    {
                        previous.authorityPendingReliableAnimGroup.clear();
                        previous.authorityPendingReliableAnimGroupTimer = 0.f;
                    }
                }
                else if (prevIt != cell.actors.end())
                {
                    prevIt->second.authorityPendingReliableAnimGroup.clear();
                    prevIt->second.authorityPendingReliableAnimGroupTimer = 0.f;
                }
                ...
                const bool stableIdlePresentation = !reliableSnapshot.currentAnimGroup.empty()
                    ...
                    && !reliableSnapshot.isDead
                    && reliableSnapshot.movementFlags == 0;
                const float presentationRefreshInterval = stableIdlePresentation ? 2.f : 1.f;
                const bool refreshDue = prevIt != cell.actors.end()
                    && prevIt->second.presentationSendTimer >= presentationRefreshInterval
                ...
                if (((presentationChanged && (hasReliablePresentationState || hadReliablePresentationState)) || refreshDue || sampleRequested))
                {
                    ...
                    if (isWatchedPresentationActor(actor, actorNetId))
                    {
                        Log(Debug::Info) << "[MPWATCH] ActorSync v2: presentation send"
                                            ...
                                            << " completion=" << reliableSnapshot.currentAnimCompletion
                                            << " sourceGroup='" << snapshot.currentAnimGroup << "'"
                                            << " sourceCompletion=" << snapshot.currentAnimCompletion
                                            << " prevSentGroup='" << previousSentAnimGroup << "'"
        ...
        if (!aiUpdate.actors.empty())
        {
            PacketActorAI aiPacket;
            aiPacket.setActorList(&aiUpdate);
            mClient.sendReliable(aiPacket.encode());
            Log(Debug::Info) << "[MP] ActorSync: sent ActorAI target update"
                             << " cell=" << aiUpdate.cellId
                             << " actors=" << aiUpdate.actors.size();
            const uint32_t localGuid = mwmp::Main::get().getPlayerSync().localPlayer().guid;
            for (const BaseActor& actor : aiUpdate.actors)
            {
                const ActorInstanceId actorNetId = actorNetIdForActorState(actor);
                const auto leaseIt = mActorAuthorityGuids.find(actorNetId);
                if (leaseIt != mActorAuthorityGuids.end() && leaseIt->second == localGuid)
                {
                    Log(Debug::Verbose) << "[MP] ActorSync: sending leased actor authority update"
                                     << " packet=ActorAI"
                                     << " actorNetId=" << actorNetId
                                     << " refId=" << actor.refId
                                     << " mpNum=" << actor.mpNum
                                     << " cell=" << aiUpdate.cellId
                                     << " owner=" << leaseIt->second;
                }
            }
        }
        ...
        bool forceCanonicalCell, bool canonicalHandoffBaseline)
    {
        ...
        // Server-spawned actors have an explicit, reliable ActorCellChange
        // transaction. Do not move their Ptr into a coordinate-derived cell
        // from an early position snapshot: under rapid reversals that snapshot
        // can arrive before the commit and make observers visibly teleport the
        // actor while its canonical cell and authority still name the source.
        // Vanilla actors retain the coordinate fallback because they do not
        // always have a spawned-actor migration identity.
        if (!forceCanonicalCell && !expectsServerSpawn
            && actor.hasAuthoritativeTransform && isExteriorActorCellId(cellId))
        {
                ...
                && findActiveCellById(worldImp, positionCellId) != nullptr)
            {
                ...
                // A reliable ActorCellChange owns canonical cell identity.
                // Buffered pre-commit transforms can still name the previous
                // cell for a few frames; never let that coordinate fallback
                // reverse the committed Ptr move. A later transform that is
                // clearly inside the canonical cell retires the guard.
                if (!actor.previousCellChangeCellId.empty())
                {
                    if (!actor.cellChangeReverseGuardLogged)
                    {
                        actor.cellChangeReverseGuardLogged = true;
                        Log(Debug::Info) << "[MP] ActorSync: suppressed resolver reverse after ActorCellChange"
                                         << " canonical=" << cellId
                ...
                {
                    if (positionCellId == cellId && borderDistance > kExteriorCellMismatchHysteresis)
                    {
                        actor.previousCellChangeCellId.clear();
                        actor.cellChangeReverseGuardTimer = 0.f;
                        actor.cellChangeReverseGuardLogged = false;
                    }
        ...
        {
            if (expectsServerSpawn && hasAuthorityForActor(actor.actorNetId, targetCellId))
        ...
        auto prepareSyncedIdleForNewBinding = [&]()
        {
            ...
            if (!actor.boundActor.isEmpty())
            {
                ...
                {
                    baseNode->setUserValue("mp_synced_idle_revision",
                        static_cast<double>(actor.syncedIdleRevision));
                    baseNode->setUserValue("mp_synced_idle_applied_revision", 0.0);
                    baseNode->setUserValue("mp_synced_idle_applied_start_point", -1.f);
                }
            }
        ...
        auto moveBoundActorToTargetCell = [&]() -> bool
        {
            ...
            {
                // ActorCellChange is a reliable canonical migration commit.  Do
                // not wait for a later ActorPositionV2 sample to resolve inside
                // the destination cell before moving the local Ptr; otherwise a
                // source-cell transform can deadlock the actor at the border.
            ...
            else if (actor.hasSmoothedPosition)
            {
                ...
                else
                {
                    if (logTrackedActor)
                    {
                        Log(Debug::Verbose) << "[MP] ActorSync: deferred cross-cell move until destination transform"
                                         << " forceCanonicalCell=" << forceCanonicalCell
                                         << " targetCell=" << targetCellId
                                         << " smoothedCell=" << exteriorCellIdForPosition(actor.smoothedPosition)
                                         << " stateCell=" << exteriorCellIdForPosition(actor.state.position)
                                         << " boundCell=" << currentBoundCellId
                                         << " previous=" << actor.previousCellChangeCellId
                                         << " reverseGuard=" << actor.cellChangeReverseGuardTimer
                                         << " refId=" << actor.state.refId
                                         << " mpNum=" << actor.state.mpNum
                                         << " actorNetId=" << actor.actorNetId
                                         << " runtimeTs=" << actor.lastServerTimestamp;
            ...
            ESM::AiSequence::AiSequence preservedAuthorityAiSequence;
            bool hasPreservedAuthorityAiSequence = false;
            bool preservedAuthorityAiWasCombat = false;
            bool preservedAuthorityAiWasPursuit = false;
            int preservedAuthorityFight = 0;
            ...
            if (preserveLocalAuthorityAi)
            {
                ...
                preservedAuthorityAiWasCombat = !preMoveStats.isDead() && preMoveAiSequence.isInCombat();
                preservedAuthorityAiWasPursuit = !preMoveStats.isDead() && preMoveAiSequence.isInPursuit();
                if (!preMoveAiSequence.isEmpty())
                {
                    preMoveAiSequence.writeState(preservedAuthorityAiSequence);
                    hasPreservedAuthorityAiSequence = !preservedAuthorityAiSequence.mPackages.empty();
                }
                preservedAuthorityFight = preMoveStats.getAiSetting(MWMechanics::AiSetting::Fight).getBase();
                preservedAuthorityHello = preMoveStats.getAiSetting(MWMechanics::AiSetting::Hello).getBase();
                preservedAuthorityFlee = preMoveStats.getAiSetting(MWMechanics::AiSetting::Flee).getBase();
                preservedAuthorityAlarm = preMoveStats.getAiSetting(MWMechanics::AiSetting::Alarm).getBase();
                Log(Debug::Info) << "[MP] ActorSync: preserving authority AI before ActorCellChange move"
                                 << " cell=" << targetCellId
                                 << " refId=" << actor.state.refId
                                 << " mpNum=" << actor.state.mpNum
                                 << " preCombat=" << preservedAuthorityAiWasCombat
                                 << " prePursuit=" << preservedAuthorityAiWasPursuit
                                 << " hasPackages=" << hasPreservedAuthorityAiSequence
            ...
            if (!expectsServerSpawn && actor.actorNetId != 0)
            {
                auto reconciledIt = mReconciledVanillaActorsByNetId.find(actor.actorNetId);
                if (reconciledIt != mReconciledVanillaActorsByNetId.end())
                    reconciledIt->second = actor.boundActor;
            }
            ...
            if (preserveLocalAuthorityAi)
            {
                ...
                const bool actorStateWasCombat = actor.state.ai.type == BaseActor::AIAction::Type::Combat;
                if (hasPreservedAuthorityAiSequence && (!actorStateWasCombat || preservedAuthorityAiWasCombat))
                {
                    postMoveAiSequence.clear();
                    postMoveAiSequence.readState(preservedAuthorityAiSequence);
                    postMoveStats.setAiSetting(MWMechanics::AiSetting::Fight, preservedAuthorityFight);
                    postMoveStats.setAiSetting(MWMechanics::AiSetting::Hello, preservedAuthorityHello);
                    postMoveStats.setAiSetting(MWMechanics::AiSetting::Flee, preservedAuthorityFlee);
                    postMoveStats.setAiSetting(MWMechanics::AiSetting::Alarm, preservedAuthorityAlarm);
                    Log(Debug::Info) << "[MP] ActorSync: restored authority AI after ActorCellChange move"
                                     << " cell=" << targetCellId
                                     << " refId=" << actor.state.refId
                                     << " mpNum=" << actor.state.mpNum
                                     << " preservedCombat=" << preservedAuthorityAiWasCombat
                                     << " preservedPursuit=" << preservedAuthorityAiWasPursuit
                                     << " packetCombat=" << actorStateWasCombat
                                     << " postCombat=" << postMoveAiSequence.isInCombat()
                                     << " packages=" << preservedAuthorityAiSequence.mPackages.size();
                }
                else if (actorStateWasCombat)
                {
                    MWWorld::Ptr combatTarget;
                    MWBase::World* combatWorld = MWBase::Environment::get().getWorld();
                        ...
                        && actor.state.ai.targetMpNum != 0)
                    {
                        for (auto& cellEntry : mCells)
                        {
                            for (auto& actorEntry : cellEntry.second.actors)
                            {
                                ActorRuntime& knownRuntime = actorEntry.second;
                                if (knownRuntime.state.mpNum == actor.state.ai.targetMpNum
                                    && !knownRuntime.boundActor.isEmpty())
                                {
                                    combatTarget = knownRuntime.boundActor;
                                    break;
                                }
                            }
                            if (!combatTarget.isEmpty())
                    ...
                    {
                        postMoveAiSequence.clear();
                        postMoveAiSequence.stack(MWMechanics::AiCombat(combatTarget), actor.boundActor);
                        int rebuiltFight = postMoveStats.getAiSetting(MWMechanics::AiSetting::Fight).getBase();
                        if (rebuiltFight < 30)
                        {
                            rebuiltFight = 30;
                            postMoveStats.setAiSetting(MWMechanics::AiSetting::Fight, rebuiltFight);
                        }
                        Log(Debug::Info) << "[MP] ActorSync: rebuilt authority combat AI after ActorCellChange move"
                                         << " cell=" << targetCellId
                                         << " refId=" << actor.state.refId
                                         << " mpNum=" << actor.state.mpNum
                                         << " targetId=" << actor.state.ai.targetId
                                         << " targetMpNum=" << actor.state.ai.targetMpNum
                                         << " targetEmpty=" << combatTarget.isEmpty()
                                         << " postCombat=" << postMoveAiSequence.isInCombat()
                                         << " fight=" << rebuiltFight;
                    }
                }
        ...
        auto isReconciledVanillaBinding = [&](const MWWorld::Ptr& ptr) -> bool
        {
            if (expectsServerSpawn || actor.actorNetId == 0 || ptr.isEmpty())
                return false;

            const auto reconciledIt = mReconciledVanillaActorsByNetId.find(actor.actorNetId);
            return reconciledIt != mReconciledVanillaActorsByNetId.end()
                && !reconciledIt->second.isEmpty()
                && (reconciledIt->second == ptr || sameLocalActorObject(reconciledIt->second, ptr))
                && ptr.getCellRef().getRefId().serializeText() == actor.state.refId;
        };

        if (!actor.boundActor.isEmpty()
            && (matchesActor(actor.boundActor, actor.state) || isReconciledVanillaBinding(actor.boundActor)))
        {
            ...
            if (mappedIt != mServerSpawnedActorsByMpNum.end())
            {
                    ...
                    && matchesActor(mappedIt->second, actor.state))
                {
                    ...
                    if (mappedCellId == targetCellId)
                    {
                        ...
                        prepareSyncedIdleForNewBinding();
                        rememberServerSpawnedActor(targetCellId, actor.boundActor, actor.state.mpNum);
                        if (logTrackedActor && !actor.bindingLogged)
                        {
        ...
        MWWorld::Ptr found;
        MWWorld::Ptr leveledSpawner;
        if (expectsV2VanillaCrossCellMove)
        {
            targetCell->forEach([&](MWWorld::Ptr ptr) -> bool
            {
                if (ptr.isEmpty() || ptr.getType() != ESM::CreatureLevList::sRecordId
        ...
        if (!leveledSpawner.isEmpty())
        {
            const auto& leveledClass
                = static_cast<const MWClass::CreatureLevList&>(leveledSpawner.getClass());
            MWWorld::Ptr localRoll = leveledClass.getSpawnedActor(leveledSpawner);
            if (!localRoll.isEmpty()
                && localRoll.getCellRef().getRefId().serializeText() == actor.state.refId)
            {
                found = localRoll;
                ...
                hasAuthorityForActor(actor.actorNetId, targetCellId), canonicalHandoffBaseline))
            {
                try
                {
                    MWWorld::ManualRef ref(
                        world->getStore(), ESM::RefId::stringRefId(actor.state.refId), 1);
                    ...
                    if (!actorTemplate.isEmpty() && actorTemplate.getClass().isActor())
                    {
                        ...
                        if (!found.isEmpty())
                        {
                            found = world->moveObject(found, targetCell,
                                osg::Vec3f(actor.state.position.pos[0], actor.state.position.pos[1],
                                    actor.state.position.pos[2]));
                        }
                        ...
                        if (!found.isEmpty())
                        {
                            ...
                            Log(Debug::Info) << "[MP] ActorSync: reconciled leveled spawner roll"
                                             << " cell=" << targetCellId
                                             << " spawnerRefNum=" << actor.state.refNum
                                             << " localRefId=" << localRefId
                                             << " authoritativeRefId=" << actor.state.refId
                                             << " actorNetId=" << actor.actorNetId;
                        }
                    }
                }
                ...
                {
                    Log(Debug::Warning) << "[MP] ActorSync: failed to reconcile leveled spawner roll"
                                        << " cell=" << targetCellId
                                        << " spawnerRefNum=" << actor.state.refNum
                                        << " authoritativeRefId=" << actor.state.refId
                                        << " error=" << e.what();
        ...
        if (found.isEmpty() && leveledSpawner.isEmpty())
        {
            targetCell->forEach([&](MWWorld::Ptr ptr) -> bool
            {
        ...
        if (found.isEmpty() && expectsV2VanillaCrossCellMove && actor.actorNetId != 0)
        {
            auto reconciledIt = mReconciledVanillaActorsByNetId.find(actor.actorNetId);
            if (reconciledIt != mReconciledVanillaActorsByNetId.end())
            {
                if (!reconciledIt->second.isEmpty()
                    && reconciledIt->second.getCell() != nullptr
                    && reconciledIt->second.getCellRef().getRefId().serializeText() == actor.state.refId)
                {
                    const std::string reconciledCellId = cellIdForPtr(reconciledIt->second);
                    if (reconciledCellId == targetCellId)
                        found = reconciledIt->second;
                    else if (!reconciledCellId.empty())
                    {
                        actor.boundActor = reconciledIt->second;
                        return moveBoundActorToTargetCell();
                    }
                }
                else
        ...
        if (found.isEmpty() && expectsV2VanillaCrossCellMove && leveledSpawner.isEmpty())
        {
        ...
        if (found.isEmpty() && expectsServerSpawn)
        {
            ...
            try
            {
                ...
                // before replicated equipment/spells are restored. ActorSync
                // already has a validated authoritative transform, so reapply
    ...
    void ActorSync::applyBoundActorState(ActorRuntime& actor)
    {
        ...
        const bool authoritativeBaseIdle = actor.state.animFlags.currentAnimGroup == "idle"
            && !actor.state.isMoving && fwdMag <= 0.1f && sideMag <= 0.1f;
        const bool hasVisualLocomotion
            = !suppressSyncedSpecialIdleLocomotion && !authoritativeBaseIdle && visualPlanarSpeed > 8.f;
        ...
        if (actor.attackControlReleaseTimer > 0.f)
        {
            bool controllerStarted = false;
            if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
            {
                baseNode->getUserValue("mp_attack_controller_started", controllerStarted);
                if (controllerStarted)
                    baseNode->setUserValue("mp_attack_controller_started", false);
            }
            actor.attackControlReleaseTimer = controllerStarted
                ? 0.f
                : std::max(0.f, actor.attackControlReleaseTimer - std::max(0.f, frameDuration));
            if (actor.attackControlReleaseTimer <= 0.f)
                stats.setAttackingOrSpell(false);
        }
        ...
        if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
        {
            ...
            {
                if (!actor.savedPuppetAiSequence && !stats.getAiSequence().isEmpty())
                {
                    auto savedSequence = std::make_shared<ESM::AiSequence::AiSequence>();
                    stats.getAiSequence().writeState(*savedSequence);
                    if (!savedSequence->mPackages.empty())
                        actor.savedPuppetAiSequence = std::move(savedSequence);
                }
        ...
        // The puppet has no active AI package, so publish the speed its
        // CharacterController would derive from the synchronized axes and actor
        // stats. This is the same Actor::getCurrentSpeed formula used by the
        // authority, without coupling cadence to interpolation corrections.
        ...
        if (isWatchedPresentationActor(actor.state, actor.actorNetId))
        {
            ...
            if (lastLogMs == 0 || nowMs - lastLogMs >= 250)
            {
                ...
                std::string activeTorsoGroup;
                std::string activeLeftArmGroup;
                std::string activeRightArmGroup;
                float activeLowerBodyCompletion = -1.f;
                float activeTorsoCompletion = -1.f;
                float activeLeftArmCompletion = -1.f;
                float activeRightArmCompletion = -1.f;
                if (MWRender::Animation* animObj = world->getAnimation(actor.boundActor))
                {
                    const auto readActiveGroup = [animObj](MWRender::BoneGroup boneGroup,
                                                     std::string& group, float& completion)
                    {
                        group = animObj->getActiveGroup(boneGroup);
                        if (!group.empty())
                            animObj->getInfo(group, &completion);
                    };
                    readActiveGroup(MWRender::BoneGroup_LowerBody,
                        activeLowerBodyGroup, activeLowerBodyCompletion);
                    readActiveGroup(MWRender::BoneGroup_Torso,
                        activeTorsoGroup, activeTorsoCompletion);
                    readActiveGroup(MWRender::BoneGroup_LeftArm,
                        activeLeftArmGroup, activeLeftArmCompletion);
                    readActiveGroup(MWRender::BoneGroup_RightArm,
                        activeRightArmGroup, activeRightArmCompletion);
                }
                std::string syncedIdleGroup;
                ...
                Log(Debug::Info) << "[MPWATCH] Actor puppet locomotion"
                                 ...
                                 << " activeLowerCompletion=" << activeLowerBodyCompletion
                                 << " activeTorso='" << activeTorsoGroup << "'"
                                 << " activeTorsoCompletion=" << activeTorsoCompletion
                                 << " activeLeftArm='" << activeLeftArmGroup << "'"
                                 << " activeLeftArmCompletion=" << activeLeftArmCompletion
                                 << " activeRightArm='" << activeRightArmGroup << "'"
                                 << " activeRightArmCompletion=" << activeRightArmCompletion;
            }
        }
        ...
        if (!actor.state.isDead && !suppressLowerBodyGroupSync)
        {
            ...
            if (!newGrp.empty() && isIdleAnimGroup(newGrp) && !isBaseIdleAnimGroup(newGrp))
            {
                if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
                {
                    baseNode->setUserValue("mp_synced_idle_group", newGrp);
                    baseNode->setUserValue("mp_synced_idle_single_cycle", actor.state.animFlags.idleSingleCycle);
                    baseNode->setUserValue("mp_synced_idle_actor_net_id",
            ...
            if (!newGrp.empty() && newGrp != actor.lastAppliedAnimGroup)
            {
                if (MWRender::Animation* animObj = world->getAnimation(actor.boundActor))
                {
                    ...
                    else
                    {
                        ...
                        const float startPoint = actor.state.animFlags.currentAnimCompletion >= 0.f
                            ? std::clamp(actor.state.animFlags.currentAnimCompletion, 0.f, 1.f)
                            : 0.f;
        ...
        if (actor.pendingAttack)
        {
            if (hitFlags == 0 && actor.animGroupHoldTimer <= 0.f)
            {
                ...
                if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
                {
                    if (isRandomCreatureAttackGroup(atkType))
                    {
                        baseNode->setUserValue("mp_attack_group", atkType);
                        baseNode->setUserValue("mp_attack_type", std::string());
                    }
                    else
                    {
                        baseNode->setUserValue("mp_attack_group", std::string());
                ...
                if (pressedChanged && actor.state.attack.pressed)
                {
                    ...
                    // Let CharacterController own the transition. Directly
                    // playing the group leaves its internal upper-body state at
                    // idle, so it can replace the attack clip in the same frame.
                    // A short press/release pulse follows the same path as local AI.
                    if (auto* baseNode = actor.boundActor.getRefData().getBaseNode())
                        baseNode->setUserValue("mp_attack_controller_started", false);
                    stats.setAttackingOrSpell(true);
                    actor.attackControlReleaseTimer = 0.75f;
                    Log(Debug::Info) << "[MP] ActorSync: queued controller attack"
                                     << " type='" << (atkType.empty() ? "slash" : atkType)
        ...
        if (actor.pendingCast)
        {
            ...
            else if (cs.release && !cs.spellId.empty())
            {
                ...
                if (effectList && (cs.targetGuid != 0 || !cs.targetRefId.empty()))
                {
                    ...
                    {
                        if (cs.targetGuid == mwmp::Main::get().getPlayerSync().localPlayer().guid)
