...
local function getCurrentSettlementName()
    ...
    if (self.cell.isExterior) then
        ...
        local intData = (cellGenStorage:get("CellGenData") or {})
...
local function getDoorDestinationStr(obj)
    local check = (cellGenStorage:get("doorData") or {})[obj.id]
    if check then
        local name = (cellGenStorage:get("cellNames") or {})[check.targetCell]
        if name then
...
local function playDoorSound(door)
    local doorSoundMap = cellGenStorage:get("doorSoundMap") or {}
...
local function getCellName(baseCellName)
    if not (cellGenStorage:get("cellNames") or {})[baseCellName] then
        return baseCellName
    else
        return (cellGenStorage:get("cellNames") or {})[baseCellName]
    end
end
