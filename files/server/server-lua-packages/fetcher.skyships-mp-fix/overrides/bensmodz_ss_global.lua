...
local hasMp, mp = pcall(require, 'openmw.mp')
...
local skyshipMerchantSetupSuppressed = false
...
local function onUpdate(dt)
  ...
  if skyshipMerchant == nil then
    if player.cell.name == "Seyda Neen" and juliusActor ~= nil and not skyshipMerchantSetupSuppressed then
      local multiplayerActive = hasMp and mp ~= nil and mp.records ~= nil and mp.records.isAvailable()
      if multiplayerActive then
        skyshipMerchantSetupSuppressed = true
        print("[Fetcher] Ben's Skyships merchant setup suppressed in multiplayer: runtime Activator/Misc record creation requires server authority.")
      else
