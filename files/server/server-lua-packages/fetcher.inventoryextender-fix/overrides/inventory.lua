...
function Inventory:create(windowType, ctx)
    ...
    if self.type == 'Inventory' then
        self.target = I.InventoryExtender.Actor()
    end
    ...
    local function getUseItemSource(row, windowType)
        ...
        return I.InventoryExtender.Actor()
    end
    ...
    local function onRowPickup(row, rowWidget, windowType, allowOutsideInterface)
        ...
        if I.InventoryExtender.Actor().type.hasEquipped and I.InventoryExtender.Actor().type.hasEquipped(I.InventoryExtender.Actor(), row.item) then
            local equipment = I.InventoryExtender.Actor().type.getEquipment(I.InventoryExtender.Actor())
            for slot, equippedItem in pairs(equipment) do
                if equippedItem == row.item then
                    equipment[slot] = nil
                    I.InventoryExtender.Actor():sendEvent("SetEquipment",{equipment=equipment})
    ...
    local function canToggleFavoriteRow(row)
        return self.target == I.InventoryExtender.Actor() and row and row.item ~= nil and not row.isBartered and not ctx.dragAndDrop.draggingObject
    end

    local function isFavoriteTransferBlocked(row)
        return self.target == I.InventoryExtender.Actor() and row and row.item ~= nil and not row.isBartered and isFavorited(row.item) and I.UI.getMode() ~= 'Interface'
    end
    ...
    local function toggleFavoriteRow(row, rowWidget)
        ...
        if state then
            if state.lastPointerRowY ~= nil then
                state.lastUsedRowPos = v2(0, state.lastPointerRowY)
    ...
    local iconRenderer = function(row, width, height, options)
        ...
        local showEnchantSwirl = row.itemRecord.enchant and shouldShowEnchantedSwirl(options.forceEnchantSwirl) or false
    ...
    local nameRenderer = function(row, width, height)
        ...
        local text = helpers.getItemName(row.item) .. (nonDraggedCount > 1 and (' (%d)'):format(nonDraggedCount) or '')
        ...
        if row.itemRecord.enchant and shouldShowEnchantedNameIcon() then
            enchanted = true
        end
    ...
    local generalColumns = {
        ...
        },
        {
            ...
            width = 80 * baseTemplates.TEXT_SIZE / 16,
    ...
    local function getAvailableCategories()
        ...
        for _, category in ipairs(categories) do
            if category.key ~= 'Favorites' or (self.target == I.InventoryExtender.Actor() and hasFavorites()) then
    ...
    local function getSyncedCategoryKey(categoryKey, targetWindow)
        if categoryKey == 'Favorites' and targetWindow and targetWindow.target ~= I.InventoryExtender.Actor() then
            return 'All'
        end
        return categoryKey
    end
            ...
            if transfer then
                if mode == 'Container' or mode == 'Companion' then
                    if self.type == 'Inventory' then
                        if ctx.windowArgs[mode] then
                            ctx.dragAndDrop:transferInto(row.item, ctx.windowArgs[mode], I.InventoryExtender.Actor(), count)
                        end
                    else
                        ctx.dragAndDrop:transferInto(row.item, I.InventoryExtender.Actor(), self.target, count, isPickpocketTake and { target = self.target } or nil)
                    end
                ...
                elseif mode == 'Interface' then
                    if not isFavorited(row.item) then
                        ctx.dragAndDrop:dropAtFeet(row.item, count)
                    else
                        ui.showMessage(l10n('UI_Msg_FavoriteItem'))
                    end
                end
    ...
    self.itemTable = ItemTable.create({
        ...
        fallbackSort = function(a, b, sortDir)
            ...
            if aEquipped ~= bEquipped then
                return aEquipped -- equipped items come first (true > false)
            end

            local aFavorited = isFavorited(a.item)
            local bFavorited = isFavorited(b.item)
            if aFavorited ~= bFavorited then
                return aFavorited -- favorited items come first (true > false)
            end
        ...
        gridItemRenderer = function(row, itemSize)
            local base = iconRenderer(row, itemSize, itemSize, {
                forceEnchantSwirl = row.itemRecord.enchant ~= nil,
            })
    ...
    if self.type == 'Inventory' then
        ...
        self.infoBar.layout.userData.addInfoLayout(specialTemplates.interactive({
            ...
            layout.userData.tooltipFn = function()
                local text
                if not configPlayer.keybinds.b_SwapUsePickup then
                    text = l10n('UI_Tooltip_UseButton')
                else
                    text = l10n('UI_Tooltip_UseButton_Swapped', { key = input.getKeyName(configPlayer.keybinds.k_PickupItem) })
                end
    ...
    if self.type == 'Container' or self.type == 'Companion' then
        ...
        if self.type == 'Container' then
            self.infoBar.layout.userData.addInfoLayout(specialTemplates.interactive({
                onClick = function()
                    if I.UI.getMode() == 'Container' and ctx.windowArgs.Container then
                        ctx.dragAndDrop:moveAll(self.target, I.InventoryExtender.Actor())
                ...
                parent = self.infoBar,
            }, baseTemplates.button(constants.Strings.DISPOSE_OF_CORPSE), ctx), function(layout)
                local visible = false
            ...
            self.infoBar.layout.userData.addInfoLayout(specialTemplates.interactive({
                onClick = function()
                    if I.UI.getMode() == 'Container' and ctx.windowArgs.Container then
                        ...
                        if not items or #items > 0 then
                            ctx.dragAndDrop:moveAll(self.target, I.InventoryExtender.Actor(), items)
                        end
                ...
                parent = self.infoBar,
            }, baseTemplates.button(constants.Strings.TAKE_ALL), ctx), function(layout)
        ...
        if self.type == 'Companion' then
            self.infoBar.layout.userData.addInfoLayout({
                ...
                layout.props.text = profit and (constants.Strings.PROFIT_VALUE .. ': ' .. tostring(profit)) or ''
                return layout
            end)
        end
        ...
        self.infoBar.layout.userData.addInfoLayout(specialTemplates.interactive({
            ...
            parent = self.infoBar,
        }, baseTemplates.button(constants.Strings.CLOSE), ctx))
                ...
                specialTemplates.interactive({
                    onClick = function()
                        ...
                        if ctx.barterState.currentBalance < 0 then
                            ctx.barterState.currentBalance = ctx.barterState.currentBalance + 1
                        else
                            ctx.barterState.currentBalance = ctx.barterState.currentBalance - 1
                        end
                        self.infoBar.layout.userData.updateAll()
    ...
    local pinned
    if self.type == 'Inventory' then
        pinned = storage.playerSection('Settings/InventoryExtender/2_WindowOptions'):get('b_InventoryWindowPinned')
    end

    local element = baseTemplates.window(
        helpers.getEquippedName(I.InventoryExtender.Actor()),
        content,
        true,
        function(layout)
            self:updateSize()
...
function Inventory:onControllerButtonPress(id)
    ...
    elseif id == input.CONTROLLER_BUTTON.Y then
        if self.ctx.dragAndDrop.draggingObject then
            local state = self.itemTable and self.itemTable.layout and self.itemTable.layout.userData.getState and self.itemTable.layout.userData.getState()
            if state and state.lastPointerRowY ~= nil then
                state.lastUsedRowPos = v2(0, state.lastPointerRowY)
            end
...
function Inventory:createData()
    ...
    local function createRowData(item, isBartered, isVirtual)
        ...
        local rowData = {
            ...
            AR = function()
                return helpers.getArmorRating(itemStack, I.InventoryExtender.Actor())
            end,
            Condition = function()
    ...
    if I.UI.getMode() == 'Barter' then
        if self.type == 'Trade' then
            for _, item in ipairs(I.InventoryExtender.Actor().type.inventory(I.InventoryExtender.Actor()):getAll()) do
...
function Inventory:updateData()
    ...
    local currentAlchemy = 0
    if types.NPC.objectIsInstance(I.InventoryExtender.Actor())==true then
        currentAlchemy = I.InventoryExtender.Actor().type.stats.skills.alchemy(I.InventoryExtender.Actor()).base
    end
...
function Inventory:setTitle()
    local title
    if self.type == 'Inventory' then
        title = helpers.getEquippedName(I.InventoryExtender.Actor())
    elseif self.target then
        title = self.target.type.record(self.target).name
    end
...
function Inventory:saveState()
    ...
    storage.playerSection('Settings/InventoryExtender/2_WindowOptions'):set(self.stateKey, dims)
    if self.type == 'Inventory' and self:isPinnable() then
        storage.playerSection('Settings/InventoryExtender/2_WindowOptions'):set('b_InventoryWindowPinned', self.element.layout.userData.pinned)
    end
end
...
function Inventory:loadState()
    ...
    local dims = storage.playerSection('Settings/InventoryExtender/2_WindowOptions'):get(self.stateKey)
    if not dims then return end
