...
function API.ChangeActor(actor)
    Actor=actor
end
...
--    else
--        print(windowName)
        --I.UI.setMode('Interface', {windows = {windowName}})
--    end
end
...
--       print("hide",windowName, arg, force)
--    else
...
local function checkCrimeLevel()
    local crimeLevel = 0
    if types.Player.objectIsInstance(I.InventoryExtender.Actor())==true then
        crimeLevel=types.Player.getCrimeLevel(I.InventoryExtender.Actor())
    end
    if lastCrimeLevel and crimeLevel < lastCrimeLevel then
        core.sendGlobalEvent('IE_TryConfiscate', { player = I.InventoryExtender.Actor(), stolenMap = windowManager.ctx.stolenItems })
    end
...
local eventHandlers = {
    ...
    IE_BarterAuthorityFailed = function(props)
        local message = 'Barter failed before payment was settled. No gold was charged.'
        if props and props.rollbackFailed then
            message = 'Barter failed and an authoritative rollback was incomplete. Reopen the merchant and report this transaction.'
        end
        ui.showMessage(message, { showInDialogue = true })
    end,
}
