...
I.Settings.registerGroup {
    key = 'SettingsInventoryExtender_fetcher',
    page = 'InventoryExtender',
    name = 'Fetcher Defaults',
    description = 'Fetcher multiplayer defaults for Inventory Extender.',
    permanentStorage = true,
    settings = {
        {
            key = 'enabled',
            name = 'Enable Inventory Extender',
            description = 'Enable Inventory Extender for this character.',
            renderer = 'checkbox',
            default = false,
        },
    },
...
local fetcherSettings = storage.playerSection('SettingsInventoryExtender_fetcher')
...
local function isEnabled()
    return fetcherSettings:get('enabled') == true and configPlayer.window.b_EnableMod == true
end
...
for _, category in ipairs(defaultCategories) do
    local categoryIcon = iconPack.getPath('categories/' .. category.key:lower() .. '.dds')
...
return {
    engineHandlers = {
        ...
        onFrame = function()
--[[
        if I.InventoryExtender.Actor().id~=self.id then
            local overrides = {
                'Inventory',
                'Trade',
                'Container',
            ...
            for _, windowName in ipairs(overrides) do
                if I.UI.isWindowVisible(windowName)==true then
                    I.UI.setMode('Interface', {windows = {}})
                    API.show(windowName, arg)
                    WindowsOpen[windowName]=true
                elseif WindowsOpen[windowName]==true and I.UI.getMode()==nil then
                    API.hide(windowName, arg)
                    WindowsOpen[windowName]=false
                end
            end
        end
]]--
